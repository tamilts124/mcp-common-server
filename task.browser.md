...
- [x] browser_get_current_url / browser_get_title convenience tools — status: tested (142/142 in test/browser-tests-part2.js)
  - notes: Trivial page.url()/page.title() wrappers — cheaper/clearer for an AI caller than writing a raw browser_evaluate script for these two common reads. Noted as a legit gap in the old task.md backlog (not superseded, unlike the rest of that list). Implementing in lib/browserActions/core.js, wiring dispatchBrowser.js/browserSchemas.js/toolsSchema.js EXEC_TOOLS/execSchemas.js pipeline enum. Tests appended to test/browser-tests-part2.js (smaller file of the two after the split).
