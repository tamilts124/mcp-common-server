"use strict";
// Standalone test script for find_insecure_random_usage (not added to the
...
const TMP = fs.mkdtempSync(path.join(os.tmpdir(), "insecure-random-"));
...
async function call(args) { return executeTool("find_insecure_random_usage", args); }

(async () => {
  console.log("find_insecure_random_usage tests:");
  ...
  writeFile("proj/a.js", [
    ...
    "const chartColor = Math.random() > 0.5 ? 'red' : 'blue';",
    "const obj = {",
    "  apiKey: Math.random().toString(16),",
    "};",
  ].join("\n"));

  await test("flags Math.random() inside sensitively-named function", async () => {
    const r = await call({ path: "proj/a.js" });
    assert.ok(r.findings.some(f => f.line === 2 && f.kind === "function-name" && f.name === "generateToken"));
  });

  await test("does NOT flag Math.random() inside innocuous function", async () => {
    const r = await call({ path: "proj/a.js" });
    assert.ok(!r.findings.some(f => f.line === 5));
  });

  await test("flags sensitively-named variable assignment", async () => {
    const r = await call({ path: "proj/a.js" });
    assert.ok(r.findings.some(f => f.line === 7 && f.kind === "assignment" && f.name === "sessionId"));
  });

  await test("does NOT flag innocuous variable assignment", async () => {
    const r = await call({ path: "proj/a.js" });
    assert.ok(!r.findings.some(f => f.line === 8));
  });

  await test("flags sensitively-named object property", async () => {
    const r = await call({ path: "proj/a.js" });
    assert.ok(r.findings.some(f => f.line === 10 && f.kind === "property" && f.name === "apiKey"));
  });

  await test("directory scan aggregates across files, sorted by file/line", async () => {
    writeFile("proj/b.js", "function createPassword() {\n  return Math.random().toString(36);\n}\n");
  ...
  await test("unrecognized single-file extension throws", async () => {
    writeFile("proj/notes.txt", "generateToken();");
    await assert.rejects(() => call({ path: "proj/notes.txt" }),
      /does not match any scanned extension/);
  });

  await test("clean file (no Math.random) yields zero findings", async () => {
    writeFile("proj/c.js", "function generateToken() { return crypto.randomUUID(); }\n");
  ...
  await test("custom extensions filter narrows directory scan", async () => {
    writeFile("proj2/d.ts", "function makeSecret() {\n  return Math.random();\n}\n");
    writeFile("proj2/e.js", "function makeSecret() {\n  return Math.random();\n}\n");
  ...
  await test("dedup: same-line function-name finding not double-counted by property pass", async () => {
    writeFile("proj3/f.js", "function generateOtp() {\n  return { code: Math.random() };\n}\n");
    const r = await call({ path: "proj3/f.js" });
    const line2 = r.findings.filter(f => f.line === 2);
    assert.strictEqual(line2.length, 1);
  });

  await test("directory containing a nested sub-dir doesn't crash", async () => {
    writeFile("proj3/ok.js", "function createAuthKey() {\n  return Math.random();\n}\n");
  ...
  await test("shell-injection-shaped content is treated as inert text, not executed", async () => {
    writeFile("proj4/g.js", "function generateSecret() {\n  return Math.random(); // '; rm -rf / #\n}\n");
  ...
  await test("max_results truncation sets truncated flag", async () => {
    let big = "function generateToken() {\n";
    for (let i = 0; i < 20; i++) big += `  const x${i} = Math.random();\n`;
    big += "}\n";
  ...
  await test("execute_pipeline op-enum registration", async () => {
    ...
    const r = await exec2("execute_pipeline", { steps: [{ op: "find_insecure_random_usage", path: "proj/a.js" }] });
