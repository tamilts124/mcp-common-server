...
/**
 * MCP Common Server â€” stdio Transport  v3.1.0
 ...
 * Zero npm dependencies â€” pure Node.js built-ins only.
 *
 ...
 * `MCP_AUTH_TOKEN` are not used here â€” stdio has no network listener and no
 ...
 *
 * â”€â”€ STDOUT GUARD â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
 ...
 *
 * â”€â”€ Message framing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
 * One JSON-RPC message per line on stdin, one JSON-RPC message per line on
 * stdout (ndjson â€” no Content-Length headers). Partial reads are buffered
 ...
 * exercised by the isolated functional test suite directly â€” this file is
 ...
 * â€” any tool added there is automatically available on both transports with
 * no changes needed here.
 */

const { READ_ONLY, ALLOW_EXEC, CMD_TIMEOUT, IGNORE_PATTERNS, CATEGORIES } = require("./lib/config");
...
console.error(`Ignore    : ${IGNORE_PATTERNS.join(", ")}`);`nconsole.error(`Categories: ${CATEGORIES.length ? CATEGORIES.join(",") : "all (no filter)"}`);
console.error(`Transport : stdio (stdin/stdout JSON-RPC, ndjson)`);
console.error(`---`);

// â”€â”€ WRITE â€” one JSON-RPC message, one line, stdout only â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function send(payload) {
  process.stdout.write(JSON.stringify(payload) + "\n");
}

// â”€â”€ READ â€” buffer stdin, dispatch each complete line â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
let buffer = "";
...
process.stdin.on("data", (chunk) => {
  ...
  for (const line of lines) {
    ...
    Promise.resolve(handleMessage(parsed.msg))
      ...
      .catch((e) => {
        // Should be unreachable â€” handleMessage already wraps executeTool in
        // its own try/catch â€” but this guarantees one malformed message can
...
process.stdin.on("end", () => {
  console.error(`[STDIO] stdin closed â€” exiting.`);
  process.exit(0);
});
