...
// Detects ruled-line (geometric) tables from two drawing conventions:
//  1. a grid of stroked rects (`x y w h re S`, the exact pattern
//     docxToPdfOps.js emits per table cell) — cells may span multiple grid
//     rows/cols (a merged cell is simply a wider/taller rect), or
//  2. a grid drawn as raw stroked line segments (`x1 y1 m x2 y2 l S`) —
//     merged cells are detected as regions where an internal dividing line
//     is simply absent (union-find over unit cells bounded by all distinct
//     line coordinates), same convention real word-processor table exports
//     use to draw merged cells.
...
// this file): only a grid whose merges form clean axis-aligned rectangles is
// recognized -- gaps, overlaps, or an irregular (non-rectangular) merged
// region fall back to flowed paragraph text (the existing pre-this-feature
// behavior, so this stays strictly additive with no regression risk);
// rotated or non-axis-aligned tables are out of scope. When every cell is
// 1x1 (no merges), the result keeps the original flat rows[][] shape for
// backward compatibility; a table with any merged cell instead returns a
// {cells:[{rowStart,rowEnd,colStart,colEnd,text}], numRows, numCols} shape
// so the docx emitter can produce w:gridSpan/w:vMerge.
function detectRectGrid(streamText) {
  ...
  if (rects.length < 2) return null; // need at least 2 rects; boundary/coverage checks below reject anything smaller than a 2x2 grid

  const round = n => Math.round(n * 4) / 4; // quarter-unit tolerance bucket
  const xsSet = new Set(), ysSet = new Set();
  for (const r of rects) {
    if (!(r.w > 0) || !(r.h > 0)) return null;
    xsSet.add(round(r.x)); xsSet.add(round(r.x + r.w));
    ysSet.add(round(r.y)); ysSet.add(round(r.y + r.h));
  }
  const xs = [...xsSet].sort((a, b) => a - b);
  const ys = [...ysSet].sort((a, b) => b - a); // descending: top row first
  const numCols = xs.length - 1, numRows = ys.length - 1;
  if (numCols < 2 || numRows < 2) return null;

  const colIdx = new Map(xs.map((x, i) => [x, i]));
  const rowIdx = new Map(ys.map((y, i) => [y, i]));
  const owner = Array.from({ length: numRows }, () => new Array(numCols).fill(-1));
  const cells = [];
  for (const r of rects) {
    const colStart = colIdx.get(round(r.x)), colEnd = colIdx.get(round(r.x + r.w));
    const rowStart = rowIdx.get(round(r.y + r.h)), rowEnd = rowIdx.get(round(r.y));
    if (colStart === undefined || colEnd === undefined || rowStart === undefined || rowEnd === undefined) return null;
    if (colStart >= colEnd || rowStart >= rowEnd) return null;
    for (let rr = rowStart; rr < rowEnd; rr++) {
      for (let cc = colStart; cc < colEnd; cc++) {
        if (owner[rr][cc] !== -1) return null; // overlapping rects: not a clean grid
        owner[rr][cc] = cells.length;
      }
    }
    cells.push({ rowStart, rowEnd, colStart, colEnd, x: xs[colStart], w: xs[colEnd] - xs[colStart], y: ys[rowEnd], h: ys[rowStart] - ys[rowEnd] });
  }
  for (let rr = 0; rr < numRows; rr++) {
    for (let cc = 0; cc < numCols; cc++) {
      if (owner[rr][cc] === -1) return null; // gap in coverage: not a clean grid
    }
  }
  return { xs, ys, numRows, numCols, cells };
}
...
function detectLineSegmentGrid(streamText) {
  ...
  while ((m = LINE_RE.exec(streamText)) !== null) {
    raw.push({ x1: Number(m[1]), y1: Number(m[2]), x2: Number(m[3]), y2: Number(m[4]) });
  }
  if (raw.length < 6) return null; // need >=3 horizontal + >=3 vertical for a 2x2 grid

  const EPS = 1.0;
  const round = n => Math.round(n * 4) / 4;
  const horiz = raw.filter(s => Math.abs(s.y1 - s.y2) < EPS && Math.abs(s.x1 - s.x2) >= EPS)
    .map(s => ({ y: round((s.y1 + s.y2) / 2), x1: Math.min(s.x1, s.x2), x2: Math.max(s.x1, s.x2) }));
  const vert = raw.filter(s => Math.abs(s.x1 - s.x2) < EPS && Math.abs(s.y1 - s.y2) >= EPS)
    .map(s => ({ x: round((s.x1 + s.x2) / 2), y1: Math.min(s.y1, s.y2), y2: Math.max(s.y1, s.y2) }));
  if (horiz.length < 3 || vert.length < 3) return null;

  const xs = [...new Set(vert.map(s => s.x))].sort((a, b) => a - b);
  const ys = [...new Set(horiz.map(s => s.y))].sort((a, b) => b - a); // descending, top first
  const numCols = xs.length - 1, numRows = ys.length - 1;
  if (numCols < 2 || numRows < 2) return null;

  const covers = (lo, hi, targetLo, targetHi) => lo <= targetLo + EPS && hi >= targetHi - EPS;

  // edgeH[row][col]: a horizontal divider exists at ys[row] spanning [xs[col],xs[col+1]]
  const edgeH = Array.from({ length: numRows + 1 }, () => new Array(numCols).fill(false));
  for (let row = 0; row <= numRows; row++) {
    const lines = horiz.filter(s => Math.abs(s.y - ys[row]) < EPS);
    for (let c = 0; c < numCols; c++) edgeH[row][c] = lines.some(s => covers(s.x1, s.x2, xs[c], xs[c + 1]));
  }
  // edgeV[row][col]: a vertical divider exists at xs[col] spanning [ys[row+1],ys[row]]
  const edgeV = Array.from({ length: numRows }, () => new Array(numCols + 1).fill(false));
  for (let col = 0; col <= numCols; col++) {
    const lines = vert.filter(s => Math.abs(s.x - xs[col]) < EPS);
    for (let r = 0; r < numRows; r++) edgeV[r][col] = lines.some(s => covers(s.y1, s.y2, ys[r + 1], ys[r]));
  }
  // outer border must be fully closed, else this isn't a complete table grid
  for (let c = 0; c < numCols; c++) if (!edgeH[0][c] || !edgeH[numRows][c]) return null;
  for (let r = 0; r < numRows; r++) if (!edgeV[r][0] || !edgeV[r][numCols]) return null;

  // union-find over unit cells: two unit cells merge into one visual cell
  // wherever the divider between them is absent
  const n = numRows * numCols;
  const parent = Array.from({ length: n }, (_, i) => i);
  const find = i => { while (parent[i] !== i) { parent[i] = parent[parent[i]]; i = parent[i]; } return i; };
  const union = (a, b) => { const ra = find(a), rb = find(b); if (ra !== rb) parent[ra] = rb; };
  const idOf = (r, c) => r * numCols + c;
  for (let r = 0; r < numRows; r++)
    for (let c = 0; c < numCols - 1; c++)
      if (!edgeV[r][c + 1]) union(idOf(r, c), idOf(r, c + 1));
  for (let r = 0; r < numRows - 1; r++)
    for (let c = 0; c < numCols; c++)
      if (!edgeH[r + 1][c]) union(idOf(r, c), idOf(r + 1, c));

  const groups = new Map();
  for (let r = 0; r < numRows; r++)
    for (let c = 0; c < numCols; c++) {
      const root = find(idOf(r, c));
  ...
  for (const members of groups.values()) {
    const rowsArr = members.map(p => p[0]), colsArr = members.map(p => p[1]);
    const rowStart = Math.min(...rowsArr), rowEnd = Math.max(...rowsArr) + 1;
    const colStart = Math.min(...colsArr), colEnd = Math.max(...colsArr) + 1;
    if (members.length !== (rowEnd - rowStart) * (colEnd - colStart)) return null; // non-rectangular merge shape: unsupported
    cells.push({ rowStart, rowEnd, colStart, colEnd, x: xs[colStart], w: xs[colEnd] - xs[colStart], y: ys[rowEnd], h: ys[rowStart] - ys[rowEnd] });
  }
  return { xs, ys, numRows, numCols, cells };
}

function mapTextOntoGrid(gridInfo, streamLineBlocks) {
  const { cells, numRows, numCols, xs, ys } = gridInfo;
  const minX = xs[0], maxX = xs[xs.length - 1], minY = ys[ys.length - 1], maxY = ys[0];
  const EPS = 1.0;
  ...
  streamLineBlocks.forEach((b, idx) => {
    ...
    for (let ci = 0; ci < cells.length; ci++) {
      const c = cells[ci];
      if (x >= c.x - EPS && x <= c.x + c.w + EPS && y >= c.y - EPS && y <= c.y + c.h + EPS) {
        const text = b.runs.map(r2 => r2.text).join("");
        texts[ci] = texts[ci] ? texts[ci] + " " + text : text;
        consumed.add(idx);
        break;
      }
    }
  ...
  const colWidths = [];
  for (let i = 0; i < numCols; i++) colWidths.push(xs[i + 1] - xs[i]);

  const merged = cells.some(c => (c.rowEnd - c.rowStart) > 1 || (c.colEnd - c.colStart) > 1);
  if (!merged) {
    // No spans: keep the original flat rows[][] shape so every existing
    // consumer/test of the non-merged case is unaffected.
    const rowsText = Array.from({ length: numRows }, () => new Array(numCols).fill(""));
    cells.forEach((c, i) => { rowsText[c.rowStart][c.colStart] = texts[i]; });
    return { rows: rowsText, colWidths, consumed };
  }
  const cellsOut = cells.map((c, i) => ({ rowStart: c.rowStart, rowEnd: c.rowEnd, colStart: c.colStart, colEnd: c.colEnd, text: texts[i] }));
  return { cells: cellsOut, numRows, numCols, colWidths, consumed };
}
...
function detectGeometricTable(streamLineBlocks, streamText) {
  ...
  return mapTextOntoGrid(gridInfo, streamLineBlocks);
}
    ...
    if (geoTable) {
      ...
      for (let idx = 0; idx < streamBlocks.length; idx++) {
        if (geoTable.consumed.has(idx)) {
          if (!inserted) {
            const tableBlock = geoTable.cells
              ? { kind: "table", cells: geoTable.cells, numRows: geoTable.numRows, numCols: geoTable.numCols, colWidths: geoTable.colWidths }
              : { kind: "table", rows: geoTable.rows, colWidths: geoTable.colWidths };
            lineBlocks.push(tableBlock);
            inserted = true;
          }
