...
// Detects ruled-line (geometric) tables: a solid rectangular grid of stroked
// rects (PDF `x y w h re S`, the exact pattern docxToPdfOps.js emits per
// table cell) with text positioned inside via Td/TD tracked in
// walkContentStream. Distinct from the pipe-delimited convention above --
// this covers real bordered tables (this project's own docx_to_pdf output,
// and any similarly-drawn PDF), not just literal "| a | b |" text.
// Documented scope (same "best-effort, not fixed" pattern as the rest of
// this file): only a perfectly solid rows*cols grid of individually-stroked
// cell rects is recognized (ragged/merged-cell layouts, and grids drawn via
// raw `m`/`l`/`S` line segments instead of `re`, are not detected -- they
// fall back to flowed paragraph text, the existing pre-this-feature
// behavior, so this is strictly additive with no regression risk); rotated
// or non-axis-aligned tables are out of scope.
function detectGeometricTable(streamLineBlocks, streamText) {
  ...
  const colWidths = xs.map((_, c) => grid[0][c].w);
