...
const MAX_WALK_UP = 64; // sane secondary cap; jailBoundary below is the real stop condition
 ...
 * `git ls-files`/`git blame` run with a cwd, and therefore read access,
 * outside the jail entirely. jailBoundary is required precisely to prevent
 * that escape; MAX_WALK_UP is only a secondary belt-and-suspenders limit.
 */
 ...
 */
function blameAuthorCounts(filePath, repoDir) {
  const cmd = `blame --porcelain -- ${q(filePath)}`;
  let raw;
  try {
    raw = gitExec(cmd, repoDir);
  } catch (e) {
    throw new Error(`git blame failed for '${filePath}': ${e.message.split("\n")[0]}`);
  }
