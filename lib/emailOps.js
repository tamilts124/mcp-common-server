...
// ── IMAP session runner ──────────────────────────────────────────────────────
// Credentials/host/port fall back to env vars (loaded from .env by lib/config.js)
// when not supplied per-call, so an agent doesn't have to pass secrets in every
// tool call. Explicit args always win over env. Env is never logged/echoed back.
function validateConnParams(args) {
  ...
  if (!username) throw new ToolError("email: 'username' is required — pass it explicitly or set EMAIL_USERNAME in .env.", -32602);
  if (!password) throw new ToolError("email: 'password' is required — pass it explicitly or set EMAIL_APP_PASSWORD in .env.", -32602);
  const host = args.host || process.env.EMAIL_HOST || "imap.gmail.com";
  const port = args.port || (process.env.EMAIL_PORT ? parseInt(process.env.EMAIL_PORT, 10) : DEFAULT_PORT);
  ...
  if (/[\r\n\x00]/.test(String(username)) || /[\r\n\x00]/.test(String(password))) {
...
function runImapSession(args, sessionFn) {
  const { username, password, host, port } = validateConnParams(args);
