...
// Parse a <w:tbl>...</w:tbl> block into { rows, colWidths }
// rows: [ [ { blocks: [paraBlock...], fill: [r,g,b]|null } ... ] ... ]
// colWidths: number[] (twips, from <w:tblGrid><w:gridCol w:w="N"/>...) or
//   null if no <w:tblGrid> is present / it doesn't declare any widths --
//   callers fall back to equal-width columns in that case.
function parseTable(tblXml) {
  const gridMatch = /<w:tblGrid\b[^>]*>([\s\S]*?)<\/w:tblGrid>/.exec(tblXml);
  let colWidths = null;
  if (gridMatch) {
    const widths = [];
    const colRe = /<w:gridCol\b[^>]*\bw:w="(\d+)"[^>]*\/?>/g;
    let cm;
    while ((cm = colRe.exec(gridMatch[1])) !== null) widths.push(Number(cm[1]));
...
function parseDocumentXml(xml) {
  ...
  while ((m = combinedRe.exec(xmlNoTables)) !== null) {
    if (m[1] !== undefined) {
      const { rows, colWidths } = parseTable(tables[Number(m[1])]);
      blocks.push({ kind: "table", rows, colWidths });
    } else {
