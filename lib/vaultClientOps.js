...
function checkStatus(res, context) {
  if (res.statusCode < 200 || res.statusCode >= 300) {
    ...
    } catch (e) { console.error("[vault_client] failed to parse JSON error body:", e.message); }
...
async function opSysHealth(args) {
  ...
  let json = {};
  if (res.raw) {
    try { json = JSON.parse(res.raw); } catch (e) { console.error("[vault_client] failed to parse health response JSON:", e.message); }
  }
  return {
    ok:              true,
