...
// and JPEG/PNG images (embedded into word/media + referenced via w:drawing,
// mirroring how docxXmlOps.js/docxToPdfOps.js read that same structure back
// out on the docx->pdf side; DCTDecode -> .jpg byte-for-byte, FlateDecode raw
// 8-bit DeviceRGB/DeviceGray samples -> re-encoded .png). Indexed/ICCBased/
// non-8-bit images and exact 2-D positioning remain documented limitations --
// this stays a flowed-text reconstruction, not a pixel-faithful layout engine.
...
function gridColsXml(colWidths, numCols) {
  if (Array.isArray(colWidths) && colWidths.length === numCols && colWidths.every(w => typeof w === "number" && isFinite(w) && w > 0)) {
    const total = colWidths.reduce((a, w) => a + w, 0);
    const TOTAL_TWIPS = 9000; // standard ~6.25in content width, matches docxToPdfOps.js's rescale target
    return colWidths.map(w => `<w:gridCol w:w="${Math.max(1, Math.round((w / total) * TOTAL_TWIPS))}"/>`).join("");
  }
  return Array.from({ length: numCols }, () => `<w:gridCol/>`).join("");
}

function cellXmlSpans(text, colSpan, vMerge) {
  const borders = `<w:tcBorders><w:top w:val="single" w:sz="4" w:color="000000"/><w:left w:val="single" w:sz="4" w:color="000000"/>` +
    `<w:bottom w:val="single" w:sz="4" w:color="000000"/><w:right w:val="single" w:sz="4" w:color="000000"/></w:tcBorders>`;
  const props = [borders];
  if (colSpan > 1) props.push(`<w:gridSpan w:val="${colSpan}"/>`);
  if (vMerge === "restart") props.push(`<w:vMerge w:val="restart"/>`);
  else if (vMerge === "continue") props.push(`<w:vMerge/>`);
  const body = vMerge === "continue" ? "<w:p/>" : `<w:p><w:r><w:t xml:space="preserve">${xmlEscape(text || "")}</w:t></w:r></w:p>`;
  return `<w:tc><w:tcPr>${props.join("")}</w:tcPr>${body}</w:tc>`;
}

// Emits a w:tbl from a merged-cell descriptor list (rowStart/rowEnd/colStart/
// colEnd/text per visual cell), producing w:gridSpan for horizontal merges
// and w:vMerge (restart/continue) for vertical merges, as detected by
// pdfRichExtractOps.js's detectGeometricTable.
function tableXmlFromCells(cellsInfo, numRows, numCols, colWidths) {
  const owner = Array.from({ length: numRows }, () => new Array(numCols).fill(-1));
  cellsInfo.forEach((c, i) => {
    for (let r = c.rowStart; r < c.rowEnd; r++)
      for (let cc = c.colStart; cc < c.colEnd; cc++) owner[r][cc] = i;
  });
  let trs = "";
  for (let r = 0; r < numRows; r++) {
    ...
    let col = 0;
    while (col < numCols) {
      const idx = owner[r][col];
      if (idx === -1) { col++; continue; } // safety: shouldn't happen for a validated grid
      const cell = cellsInfo[idx];
      const colSpan = cell.colEnd - cell.colStart;
      const rowSpan = cell.rowEnd - cell.rowStart;
      row += cell.rowStart === r
        ? cellXmlSpans(cell.text, colSpan, rowSpan > 1 ? "restart" : null)
        : cellXmlSpans("", colSpan, "continue");
      col = cell.colEnd;
    }
    trs += `<w:tr>${row}</w:tr>`;
  }
  return `<w:tbl><w:tblPr><w:tblBorders/></w:tblPr><w:tblGrid>${gridColsXml(colWidths, numCols)}</w:tblGrid>${trs}</w:tbl>`;
}

function tableXml(rows, colWidths) {
  const grid = Math.max(1, ...rows.map(r => r.length));
  let gridCols;
  if (Array.isArray(colWidths) && colWidths.length === grid && colWidths.every(w => typeof w === "number" && isFinite(w) && w > 0)) {
    const total = colWidths.reduce((a, w) => a + w, 0);
    const TOTAL_TWIPS = 9000; // standard ~6.25in content width, matches docxToPdfOps.js's rescale target
    gridCols = colWidths.map(w => `<w:gridCol w:w="${Math.max(1, Math.round((w / total) * TOTAL_TWIPS))}"/>`).join("");
  } else {
    gridCols = Array.from({ length: grid }, () => `<w:gridCol/>`).join("");
  }
...
function pdfToDocx(absSrc, origSrc, absDest, origDest) {
  ...
  for (const b of blocks) {
    if (b.kind === "table") {
      bodyXml += Array.isArray(b.cells)
        ? tableXmlFromCells(b.cells, b.numRows, b.numCols, b.colWidths)
        : tableXml(b.rows, b.colWidths);
      tables++;
    } else if (b.kind === "image") {
      imgCounter++;
      const relId = `rIdImg${imgCounter}`;
      const ext = b.imageKind === "png" ? "png" : "jpg";
      const mediaName = `image${imgCounter}.${ext}`;
  ...
  const contentTypes = CONTENT_TYPES_HEAD +
    ...
    (typeOverrides.has("png") ? `<Default Extension="png" ContentType="image/png"/>` : "") +
...
module.exports = { pdfToDocx, tableXmlFromCells };
