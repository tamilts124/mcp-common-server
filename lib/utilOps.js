...
function queryJson(filePath, query) {
  ...
  if (!query || query.trim() === "") {
    return { value: parsed, path: ".", type: Array.isArray(parsed) ? "array" : typeof parsed };
  }
  ...
  for (const part of parts) {
    if (cursor === undefined || cursor === null || typeof cursor !== "object")
      throw new Error(`Path '${query}' does not exist in the JSON document (stopped at '${part}').`);
    cursor = cursor[part];
  }
...
module.exports = { fileChecksum, zipDirectory, queryJson };
