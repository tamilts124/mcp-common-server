...
function extractObjectInfo(buf, addr, offsetSize, lengthSize) {
  ...
  for (const msg of header.messages) {
    try {
      ...
      const msg = `[hdf5_client] failed to parse object header message; attribute may be missing: ${e.message}`;
      console.error(msg);
    }
  }
...
// Group traversal
// ────────────────────────────────────────────────────────────────────────────
function traverseGroup(buf, groupAddr, offsetSize, lengthSize, depth, parentPath, _warnings = []) {
  if (depth > MAX_DEPTH) return [];
      ...
      try {
        ...
        if (link && link.name) {
          ...
          if (link.linkType === 0 && link.targetAddr !== null && !isUndefined(link.targetAddr, offsetSize)) {
            ...
            if (objT === 'group') {
              items.push(...traverseGroup(buf, link.targetAddr, offsetSize, lengthSize, depth + 1, itemPath, _warnings));
            }
        ...
        const msg = `[hdf5_client] failed to parse v1 group link entry; item may be missing: ${e.message}`;
        console.error(msg); _warnings.push(msg);
      }
    }
  }
    ...
    if (msg.type === 17 || msg.type === 1 + 16) {
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (objT === 'group') {
            items.push(...traverseGroup(buf, entry.headerAddr, offsetSize, lengthSize, depth + 1, itemPath, _warnings));
          }
        ...
        const msg = `[hdf5_client] failed to parse symbol table entry; item may be missing: ${e.message}`;
        console.error(msg); _warnings.push(msg);
      }
    }
  }
    ...
    for (const msg of header.messages) {
      if (msg.type === 6) {
        try {
          ...
          console.error("[hdf5_client] failed to parse link message during path traversal:", e.message);
        }
      }
    }
    if (!found) {
...
function opInfo(args, resolved) {
  ...
  const _warnings = [];
  const items = traverseGroup(buf, sb.rootAddress, sb.offsetSize, sb.lengthSize, 0, '', _warnings);
...
function opList(args, resolved) {
  ...
  const _warnings = [];
  const items = traverseGroup(buf, groupAddr, sb.offsetSize, sb.lengthSize, 0, groupPath === '/' ? '' : groupPath, _warnings);
