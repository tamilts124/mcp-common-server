...
const { runNpmScript } = require("./runNpmScriptOps");
...
const { httpDownload } = require("./httpDownloadOps");
const { jsonPathSet } = require("./jsonPathSetOps");
...
const WRITE_DISPATCH = {
  ...
  send_process_input(args) {
    return sendProcessInput(args);
  },

  run_npm_script(args) {
    const { resolved, alias } = resolveClientPath(args.path || ".");
    const origPath = args.path || alias || ".";
    return runNpmScript(resolved, origPath, args.script, {
  ...
  extract_tar(args) {
    const { resolved: tarResolved } = resolveClientPath(args.path);
    const { resolved: destResolved } = resolveClientPath(args.destination);
    const result = extractTar(tarResolved, destResolved, { overwrite: !!args.overwrite });
    return { source: args.path, destination: args.destination, ...result };
  },
  ...
  json_merge(args) {
    const { resolved } = resolveClientPath(args.path);
    return jsonMerge(resolved, args.path, args.patch, { apply: args.apply, indent: args.indent });
  },
  ...
  http_download(args) {
    const { resolved } = resolveClientPath(args.destination);
    return httpDownload(args.url, resolved, args.destination, {
      headers: args.headers, timeout: args.timeout, max_bytes: args.max_bytes,
    });
  },

  json_path_set(args) {
    const { resolved } = resolveClientPath(args.path);
    return jsonPathSet(resolved, args.path, args.query, {
      value: args.value, delete: args.delete, format: args.format,
      apply: args.apply, indent: args.indent,
    });
  },
