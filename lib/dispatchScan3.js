...
const { findSqlInjectionRisk } = require("./sqlInjectionOps");
const { findCommandInjectionRisk } = require("./commandInjectionOps");
const { findXssRisk } = require("./xssRiskOps");
const { findPathTraversalRisk } = require("./pathTraversalOps");
...
const SCAN_DISPATCH_3 = {
  ...
  find_sql_injection_risk(args) {
    const { resolved } = resolveClientPath(args.path || '.');
    const origPath = args.path || '.';
    return findSqlInjectionRisk(resolved, origPath, {
      extensions: args.extensions,
      maxResults: args.max_results,
    });
  },

  find_command_injection_risk(args) {
    const { resolved } = resolveClientPath(args.path || '.');
    const origPath = args.path || '.';
    return findCommandInjectionRisk(resolved, origPath, {
      extensions: args.extensions,
      maxResults: args.max_results,
    });
  },
  ...
  find_xss_risk(args) {
    const { resolved } = resolveClientPath(args.path || '.');
    const origPath = args.path || '.';
    return findXssRisk(resolved, origPath, {
      extensions: args.extensions,
      maxResults: args.max_results,
    });
  },

  find_path_traversal_risk(args) {
    const { resolved } = resolveClientPath(args.path || '.');
    const origPath = args.path || '.';
    return findPathTraversalRisk(resolved, origPath, {
      extensions: args.extensions,
      maxResults: args.max_results,
    });
  },
...
const { SCAN_DISPATCH_4 } = require('./dispatchScan4');
Object.assign(SCAN_DISPATCH_3, SCAN_DISPATCH_4);
