...
// ── Mouse / keyboard helpers (Windows only) ───────────────────────────────────

function _mouseClick(x, y, button = "left") {
  if (process.platform !== "win32")
    throw new ToolError("screen_capture mouse_click: only supported on Windows currently.", -32602);
  const downFlag = button === "right" ? 8  : 2;
  const upFlag   = button === "right" ? 16 : 4;
  const ps = [
    "Add-Type -AssemblyName System.Windows.Forms",
...
function _sendKeys(rawInput) {
  if (process.platform !== "win32")
    throw new ToolError("screen_capture send_keys: only supported on Windows currently.", -32602);

  // Parse multi-key combo notation into SendKeys string
...
function _mouseMove(x, y) {
  if (process.platform !== "win32")
    throw new ToolError("screen_capture mouse_move: only supported on Windows currently.", -32602);
  const ps = [
    "Add-Type -AssemblyName System.Windows.Forms",
    `[System.Windows.Forms.Cursor]::Position = New-Object System.Drawing.Point(${x}, ${y})`,
  ];
