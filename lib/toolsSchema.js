"use strict";
// â”€â”€ TOOL JSON-SCHEMA DEFINITIONS â€” thin aggregator â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Individual schema groups live under lib/schemas/ to keep every file under
// the project's 500-line threshold. This file just concatenates them in the
// same order they used to appear in the monolithic version of this file.

const { CORE_READ_SCHEMAS } = require("./schemas/coreSchemas");
const { GIT_SCHEMAS }       = require("./schemas/gitSchemas");
const { UTIL_SCHEMAS }      = require("./schemas/utilSchemas");
const { WRITE_SCHEMAS }     = require("./schemas/writeSchemas");
const { EXEC_SCHEMAS }      = require("./schemas/execSchemas");
const { BROWSER_SCHEMAS }   = require("./schemas/browserSchemas");
const { EMAIL_SCHEMAS }     = require("./schemas/emailSchemas");
const { SQLITE_SCHEMAS }    = require("./schemas/sqliteSchemas");
const { COMMUNITY_SCHEMAS } = require("./schemas/communitySchemas");
const { GH_SCHEMAS }        = require("./schemas/ghSchemas");

const TOOLS_ALL = [
  ...CORE_READ_SCHEMAS,
  ...GIT_SCHEMAS,
  ...UTIL_SCHEMAS,
  ...WRITE_SCHEMAS,
  ...EXEC_SCHEMAS,
  ...BROWSER_SCHEMAS,
  ...EMAIL_SCHEMAS,
  ...SQLITE_SCHEMAS,
  ...COMMUNITY_SCHEMAS,
  ...GH_SCHEMAS,
];

// Tool category sets
const WRITE_TOOLS = new Set([
  "write_file", "write_files", "create_file", "create_files",
  "delete_file", "delete_files", "move_file", "copy_file",
  "move_directory", "copy_directory",
  "create_directory", "delete_directory", "replace_in_file",
  "truncate_file", "append_file",
  "base64_decode", "json_format", "text_transform",
  "json_patch", "apply_patch", "unzip_archive", "extract_tar", "yaml_patch", "yaml_merge", "json_merge", "convert_data", "csv_convert",
  "gzip_compress", "gzip_decompress", "brotli_compress", "brotli_decompress",
  "md_to_docx", "docx_to_md", "md_to_pdf", "pdf_to_md", "docx_to_pdf", "pdf_to_docx", "http_download", "json_path_set",
  // zip_directory's schema lives in UTIL_SCHEMAS and its handler in
  // dispatchRead.js (grouped with the other utility tools), but it writes a
  // real .zip file to disk (mkdirSync + fs.writeFileSync in zipDirectory()),
  // so it must still be gated here -- dispatch file/schema-group location is
  // unrelated to write-gating, only membership in this Set is.
  "zip_directory", "create_tar",
]);

const EXEC_TOOLS = new Set([
  "run_command", "execute_pipeline", "run_npm_script",
  "start_process", "get_process_output", "kill_process", "list_processes", "send_process_input",
  // Browser tools spawn a real Chromium process, same trust tier as exec.
  "browser_launch", "browser_navigate", "browser_get_content", "browser_evaluate",
  "browser_click", "browser_type", "browser_screenshot", "browser_get_console_logs",
  "browser_list_sessions", "browser_close",
  "browser_wait_for_selector", "browser_get_current_url", "browser_get_title", "browser_go_back", "browser_go_forward", "browser_reload",
  "browser_get_cookies", "browser_set_cookies", "browser_pdf", "browser_select_option", "browser_press_key",
  "browser_wait_for_navigation", "browser_hover", "browser_upload_file",
  "browser_scroll", "browser_double_click", "browser_right_click", "browser_drag_and_drop", "browser_download",
  "browser_get_attribute", "browser_is_visible", "browser_is_checked", "browser_check", "browser_uncheck",
  "browser_get_element_info",
  "browser_new_page", "browser_switch_page", "browser_list_pages", "browser_close_page",
  "browser_network_start", "browser_network_stop", "browser_get_network_requests",
  "browser_route", "browser_unroute",
  "browser_emulate", "browser_set_viewport",
  "browser_set_extra_headers", "browser_get_local_storage", "browser_set_local_storage",
  "browser_add_init_script", "browser_get_page_metrics",
  "browser_expose_function", "browser_get_exposed_calls", "browser_wait_for_response",
  "browser_get_storage_state",
  "browser_accessibility_snapshot", "browser_find_by_role",
  "browser_handle_next_dialog", "browser_get_dialog_log", "browser_wait_for_dialog",
  "browser_list_frames", "browser_frame_click", "browser_frame_type", "browser_frame_get_content", "browser_frame_evaluate",
  "browser_start_recording", "browser_stop_recording", "browser_get_recording", "browser_clear_recording", "browser_replay_actions",
  // SQLite tools execute arbitrary SQL, same trust tier as run_command/browser_*.
  "sqlite_create", "sqlite_connect", "sqlite_execute", "sqlite_disconnect", "sqlite_connections", "sqlite_tables",
  // git_write_ops spawns git via spawnSync â€” same exec trust tier as run_command.
  "git_write_ops",
  // gh_client spawns the gh CLI via spawnSync - same exec trust tier.
  "gh_client",
]);


const CATEGORY_TOOLS = new Map([
  ["read",      new Set(CORE_READ_SCHEMAS.map(t => t.name))],
  ["git",       new Set([...GIT_SCHEMAS, ...GH_SCHEMAS].map(t => t.name))],
  ["util",      new Set(UTIL_SCHEMAS.map(t => t.name))],
  ["write",     new Set(WRITE_SCHEMAS.map(t => t.name))],
  ["exec",      new Set(EXEC_SCHEMAS.map(t => t.name))],
  ["browser",   new Set(BROWSER_SCHEMAS.map(t => t.name))],
  ["email",     new Set(EMAIL_SCHEMAS.map(t => t.name))],
  ["sqlite",    new Set(SQLITE_SCHEMAS.map(t => t.name))],
  ["community", new Set(COMMUNITY_SCHEMAS.map(t => t.name))],
]);

/**
 * Return the subset of TOOLS_ALL that belongs to any of the requested
 * categories. Pass null/undefined/empty array to return all tools.
 * Unknown category names are silently ignored (logged to stderr).
 */
function filterByCategories(categories) {
  if (!categories || categories.length === 0) return TOOLS_ALL;
  const allowed = new Set();
  for (const cat of categories) {
    const set = CATEGORY_TOOLS.get(cat.toLowerCase());
    if (set) { for (const n of set) allowed.add(n); }
    else { process.stderr.write(`[config] Unknown --category value: ${cat} (ignored)\n`); }
  }
  return TOOLS_ALL.filter(t => allowed.has(t.name));
}

module.exports = { TOOLS_ALL, WRITE_TOOLS, EXEC_TOOLS, CATEGORY_TOOLS, filterByCategories };
