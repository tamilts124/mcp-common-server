...
//   2026-07-16 Bug Fix 3 (ROOT CAUSE of column mis-mapping): For tables with an
//              INTEGER PRIMARY KEY column, SQLite writes serial-type 0 (null) as a
//              phantom placeholder in the record payload header for that column, so
//              values[] = [null(phantom), created_on, job_title, ...]. The previous
//              code started valIdx=0, mapping values[0](null) → created_on and
//              values[1](created_on) → job_title — every column was off by one.
//              Fix: start valIdx=1 when hasRowidPK is true, skipping the phantom.
...
function executeSelect(parsed, schema, dbBuf, pageSize, encoding) {
  ...
  // Detect INTEGER PRIMARY KEY — this column IS the rowid alias and is NOT stored as a
  // real value in the record payload. Instead, SQLite writes serial-type 0 (null) as a
  // placeholder in the header for that column slot, so the payload values[] array has:
  //   values[0]  = null  (the phantom rowid placeholder — must be SKIPPED)
  //   values[1]  = first non-PK column value  (e.g. created_on)
  //   values[2]  = second non-PK column value (e.g. job_title)
  //   ...
  // Without skipping values[0], every column is off-by-one: created_on gets job_title's
  // data, job_title gets job_desc's data, and so on. This was the root-cause bug.
  ...
  let objRows = rawRows.map(row => {
    const obj = {};
    if (hasRowidPK) {
      // values[0] is the phantom null placeholder for the rowid column — skip it.
      // Real column data starts at values[1].
      let valIdx = 1;
      for (let i = 0; i < colNames.length; i++) {
        if (colNames[i] === pkCol.name) {
          // Inject the actual rowid (not from values[])
