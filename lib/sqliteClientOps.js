...
// Parse SELECT statement: SELECT [cols] FROM [table] [WHERE ...] [LIMIT n] [OFFSET m]
function parseSelect(sql) {
  ...
  if (peek()?.value === "*") {
    ...
    while (pos < tokens.length) {
      const t = peek();
      if (!t || t.value.toUpperCase() === "FROM") break;
      if (t.value === ",") { consume(); continue; }
      const col = consume();
...
function evaluateWhere(whereToks, rowObj) {
  ...
  // Convert tokens to expression string and evaluate simply
  // Support: col OP value AND/OR col OP value ...
  try {
    // Build evaluation by iterating token triplets (col, op, value)
    // and combining with AND/OR
    let result = true;
    let combinator = "AND";
    let i = 0;
...
// ══════════════════════════════════════════════════════════════════════════════
// WRITE OPERATIONS: execute INSERT/UPDATE/DELETE using WAL or direct page write
// (We implement a simple in-memory modification + file rewrite approach)
// ══════════════════════════════════════════════════════════════════════════════
