...
async function launchSession(args = {}) {
  ...
  if (args.proxy && typeof args.proxy === "object" && args.proxy.server) {
    ...
    // For SOCKS5, route DNS through the proxy tunnel by default so that
    // .onion and other non-public hostnames resolve correctly.
    ...
    if (isSocks5 && dnsThroughProxy) {
      ...
      extraChromiumArgs.push("--host-resolver-rules=MAP * ~NOTFOUND , EXCLUDE localhost");
    }
  }
