...
const CORE_READ_SCHEMAS = [
  ...
  },
  {
    ...
    description: "Validate a JSON data file against a JSON Schema file. Hand-rolled subset validator (zero-dependency, no ajv): supports type (incl. 'integer'), required, properties, additionalProperties: false, items (array element schema), enum, minimum/maximum, minLength/maxLength, pattern, minItems/maxItems, applied recursively. Not a full JSON Schema draft implementation (no $ref, allOf/anyOf/oneOf, formats) — good enough for validating config files or tool-output shapes. Collects every violation rather than stopping at the first. Read-only — does not require MCP_ALLOW_EXEC. Returns { path, schemaPath, valid, errorCount, errors: [{path, message}] } where errors[].path is a dot/bracket path into the data (e.g. 'items[2].name').",
    inputSchema: { type: "object", required: ["path", "schema_path"], properties: {
      path:        { type: "string", description: "Path to the JSON data file to validate." },
      schema_path: { type: "string", description: "Path to the JSON Schema file to validate against." },
    }},
  },
  {
    name: "json_schema_generate",
    description: "Infer a minimal JSON Schema describing a sample JSON/YAML document's shape — the read-only counterpart to json_schema_validate, for scaffolding a schema from a real example instead of hand-writing one. Objects -> {type:'object', properties, required} where every own key present in the sample is marked required (a single sample can't know which keys are optional — edit the generated 'required' array by hand if some are known to be optional). Arrays -> {type:'array', items} where items is the MERGED schema of up to max_array_sample sampled elements: object elements merge properties as a UNION of keys with required as the INTERSECTION (only keys present in every sampled element); mixed scalar/object/array types across elements produce {type:[...]} (JSON Schema's multi-type array form) with no further narrowing. Scalars map to 'string'/'boolean'/'integer'/'number'/'null' ('integer' vs 'number' via Number.isInteger). Format is auto-detected from the file extension ('.json' -> JSON, '.yaml'/'.yml' -> YAML) or forced via 'format'. Read-only — does not require MCP_ALLOW_EXEC. Returns { path, format, schema }.",
    inputSchema: { type: "object", required: ["path"], properties: {
      path:             { type: "string", description: "Path to the sample JSON or YAML file to infer a schema from." },
      ...
      max_array_sample: { type: "number", description: "Max array elements sampled per array when inferring/merging an items schema (default 100, hard cap 1000)." },
    }},
  },
  {
    name: "env_diff",
