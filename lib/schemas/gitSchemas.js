...
const GIT_SCHEMAS = [
  ...
  {
    name: "check_dotenv_files_not_gitignored",
    description: "Finds `.env`/`.env.<name>` files on disk (excluding `.example`/`.sample`/`.template`/`.dist` variants, which are meant to be committed) and cross-checks each against real git state via `git ls-files` (batched) and `git check-ignore -v --no-index`. Flags two distinct risk shapes: `dotenv_file_tracked_by_git` (error) — the file is already committed, so its contents are exposed in repo history regardless of any later .gitignore rule or `git rm`; `dotenv_file_not_gitignored` (warning) — not tracked yet, but no .gitignore rule covers it either, one `git add .` away from being committed. A dotenv file that's both untracked and gitignore-covered produces no finding. Distinct from check_gitignore_coverage (checks a generic representative path list, doesn't look at whether a real dotenv file already sitting in the repo is tracked). Returns { path, filesScanned, filesSkippedAsExample, trackedCount, notIgnoredCount, findingsCount, errorCount, warningCount, truncated, dotenvFiles: [{file,tracked,ignored}], findings: [{file,rule,severity,message}] }. Always available — does not require `MCP_ALLOW_EXEC`.",
    inputSchema: { type: "object", properties: {
      path: { type: "string", description: "Any path inside the git repository (used to locate the repo). Defaults to first root." },
      include_examples: { type: "boolean", description: "If true, also scan .example/.sample/.template/.dist variants (default false)." },
      max_files: { type: "number", description: "Cap on candidate dotenv files processed (1-500, default 200)." },
    }},
  },
  {
...

  {
    ...
    description: "Combine scan_todos (TODO/FIXME/HACK/XXX/BUG comment-marker detection) with the same per-line git blame lookup used by find_todo_owners, but flags by *age* instead of author: any marker whose blamed commit date is at or beyond threshold_days old is reported stale (likely-abandoned), sorted oldest-first by ageDays. A marker whose line can't be blamed (uncommitted/untracked file, etc.) is listed in `unresolved` with a reason rather than aborting the scan. Requires the target to be inside a git repository. Returns { path, thresholdDays, totalMarkers, staleCount, freshCount, unresolvedCount, truncated, stale: [{file, line, marker, text, author, date, ageDays}], unresolved: [{file, line, marker, reason}] }. Always available — does not require MCP_ALLOW_EXEC.",
    inputSchema: { type: "object", required: ["path"], properties: {
      path:            { type: "string",  description: "File or directory (inside a git repository) to scan for TODO-style markers." },
      threshold_days:  { type: "number",  description: "Age cutoff in days — markers blamed at or beyond this age are 'stale' (default 90, min 1)." },
      markers:         { type: "array", items: { type: "string" }, description: "Marker words to look for (default: TODO, FIXME, HACK, XXX, BUG)." },
      extensions:      { type: "array", items: { type: "string" }, description: "Directory mode only: restrict to files with these extensions, e.g. ['.js', '.ts']." },
      case_sensitive:  { type: "boolean", description: "Case-sensitive marker matching (default: false)." },
      max_markers:     { type: "number",  description: "Cap on markers processed — each costs one git blame call (1-1000, default 200)." },
  ...
  {
    name: "git_worktree_prune_candidates",
    description: "Filter `git_worktree_list` down to worktrees that are safe `git worktree prune` candidates — composed on top of that tool's already-tested parsing, no new git plumbing. A worktree is a candidate if git itself marked it `prunable` (e.g. its directory was deleted or its administrative files are stale) OR its directory is missing from disk (a second cross-check signal, since git doesn't always flag this immediately). The main worktree is never a candidate. Returns { totalWorktrees, candidateCount, candidates: [{path, branch, missingOnDisk, isPrunable, prunableReason}] }. Read-only, does not run `git worktree prune` itself. Always available — does not require MCP_ALLOW_EXEC.",
    inputSchema: { type: "object", properties: {
      path: { type: "string", description: "Any path inside the git repository (used to locate the repo). Defaults to first root." },
    }},
  },
  ...
  },
  {
    ...
    description: "Scans repo tags (via git_tag_list), keeps only semver-shaped ones (a leading non-digit prefix like 'v' or 'release-' is stripped before parsing), finds the highest by real semver precedence (not string/date sort), and proposes the next patch/minor/major version. Optionally cross-checks package.json's own 'version' field against the latest tag to flag drift — best-effort, omitted if package.json is missing/unreadable/has no version field. Returns { path, tagCount, matchedCount, latestTag, latestVersion, suggestions: {patch,minor,major}, pkgJsonVersion, inSyncWithPackageJson } (suggestions/latestTag/latestVersion are null plus a `note` when no semver-shaped tags exist). Always available — does not require MCP_ALLOW_EXEC.",
    inputSchema: { type: "object", properties: {
      path: { type: "string", description: "Any path inside the git repository (used to locate the repo, and as the default directory to look for package.json). Defaults to first root." },
      pkg_path: { type: "string", description: "Explicit path to a package.json to cross-check (default: 'package.json' inside the resolved repo path)." },
    }},
  },
  {
    name: "git_tag_list",
  ...
  {
    name: "git_object_count",
    description: "Wrap `git count-objects -v` for a fast repo object-database health signal: loose-object count/size, pack count/size, and prune/garbage candidates, without walking history. Useful before larger operations (clone, push, bundle, or advising whether `git gc` would help) since it's a single cheap call rather than scanning commits. All size fields are reported in KiB (git's own convention for this command's output, kept honest as sizeKb rather than implying bytes) with sizeHuman companions. Fields git omits on older versions (prune-packable/garbage/size-garbage) default to 0 rather than being missing. `gcRecommended` is a heuristic (count > 200, or any prune-packable/garbage objects), not a hard rule. Returns { count, sizeKb, sizeHuman, inPack, packs, sizePackKb, sizePackHuman, prunePackable, garbage, sizeGarbageKb, sizeGarbageHuman, gcRecommended, raw }. Read-only — does not require MCP_ALLOW_EXEC beyond running git itself.",
    inputSchema: { type: "object", properties: {
      path: { type: "string", description: "Any path inside the git repository (used to locate the repo). Defaults to first root." },
    }},
  },
  ...
  {
    name: "find_recent_force_pushes",
    description: "Detect history rewrites (force-pushes or equivalent) that the local reflog has observed for a ref — typically a remote-tracking ref such as `origin/main`, which is updated locally on every `fetch`/`pull`. Walks consecutive reflog entries and checks each pair with `git merge-base --is-ancestor <old> <new>`: if the older entry's commit is NOT an ancestor of the newer one, that update rewrote history instead of fast-forwarding it — the local signature of a force-push. This only sees rewrites the local reflog actually recorded (bounded by reflog expiry and the entries scanned), not a full remote-side audit — a fast local warning signal, not a complete history of every push. Reuses `git_reflog`'s own tested parsing/error handling internally (same ref-blank-falls-back-to-HEAD and limit-clamping behavior). Returns { ref, entriesScanned, rewritesDetected, events: [{ oldHash, oldShortHash, newHash, newShortHash, oldDate, newDate, action }] }. Always available — does not require MCP_ALLOW_EXEC.",
    inputSchema: { type: "object", properties: {
      path:  { type: "string", description: "Any path inside the git repository (used to locate the repo). Defaults to first root." },
      ref:   { type: "string", description: "Ref whose reflog to scan (default 'HEAD'). Pass a remote-tracking ref like 'origin/main' for the actual force-push-detection use case." },
      limit: { type: "number", description: "Max reflog entries to scan (1–500, default 30)." },
    }},
  },
  {
    name: "check_stash_apply_risk",
    description: "Check whether a stash would apply cleanly, WITHOUT touching the working tree, index, or the stash itself. Extracts the stash's own diff via `git stash show -p` and validates it against the current working tree with `git apply --check` (a pure dry-run). Complements `merge_conflict_risk` (branch-vs-branch) and `git_stash_list` (enumeration only) — neither answers whether un-stashing right now would conflict. Returns { stash, applyClean, conflictOutput }: conflictOutput is null when applyClean is true, otherwise git's own rejection reason. Always available — does not require MCP_ALLOW_EXEC.",
    inputSchema: { type: "object", properties: {
      path:  { type: "string", description: "Any path inside the git repository (used to locate the repo). Defaults to first root." },
      stash: { type: "string", description: "Stash to check, in git's own 'stash@{N}' form (default 'stash@{0}', the most recent stash)." },
    }},
  },
  {
    name: "git_blame_ownership_diff",
    description: "Compare per-author git-blame line-ownership share of a single file between two refs, surfacing ownership shifts a plain git_diff (line-level, not attribution) or a single git_ownership call (one point in time) don't show directly — e.g. a wholesale reformat can silently transfer majority ownership to whoever ran it even though the underlying logic is unchanged. Reuses git_ownership's tested blame parser at each ref. Returns { path, refA, refB, ownershipA, ownershipB, shifts: [{name, percentageA, percentageB, delta}], maxShiftAuthor, maxShiftDelta } sorted by absolute delta descending. path must be a single file (not a directory). Always available — does not require MCP_ALLOW_EXEC.",
    inputSchema: { type: "object", properties: {
      path:  { type: "string", description: "Path to a single tracked file." },
      ref_a: { type: "string", description: "Older ref to compare from (e.g. a tag, an earlier commit, 'HEAD~20'). Required." },
      ref_b: { type: "string", description: "Newer ref to compare to (default 'HEAD')." },
    }, required: ["path", "ref_a"]},
  },
  {
    name: "git_tag_annotate_audit",
    description: "Flag tags that skip release-hygiene annotation: lightweight tags (a bare ref with no tagger metadata of its own) and annotated tags with an empty/whitespace-only message. Composed directly on git_tag_list's tested for-each-ref parsing. Returns { totalTags, annotatedCount, lightweightCount, flaggedCount, tags: [{name, hash, isAnnotated, message, flagged, reason: 'lightweight'|'annotated-empty-message'|null}] }. Always available — does not require MCP_ALLOW_EXEC.",
    inputSchema: { type: "object", properties: {
      path: { type: "string", description: "Any path inside the git repository (used to locate the repo). Defaults to first root." },
    }},
  },
  {
    name: "check_commit_signatures",
    description: "Report GPG/SSH signature status for the most recent commits on a ref, using git's own %G? classification from a single git log call (not a per-commit git verify-commit process). Status codes: G=good, U=good/unknown validity, X=good but expired, Y=good but expired key, R=good but revoked key (treated as bad), B=bad signature, E=could not be checked, N=no signature. Returns { ref, totalScanned, signedCount, unsignedCount, badCount, commits: [{hash, shortHash, status, statusMeaning, signed, bad, signer, subject}] }. Always available — does not require MCP_ALLOW_EXEC (GPG itself is not required either; unsigned/unverifiable commits simply report N/E).",
    inputSchema: { type: "object", properties: {
      path:  { type: "string", description: "Any path inside the git repository (used to locate the repo). Defaults to first root." },
      ref:   { type: "string", description: "Ref to walk (default 'HEAD')." },
      limit: { type: "number", description: "Max commits to scan (1–200, default 20)." },
    }},
  },
  {
    name: "git_commit_frequency",
    description: "Commits-per-day (and per-author) activity histogram over a recent lookback window, from a single `git log --since` call bucketed client-side by UTC day — no per-file cost, unlike git_blame_hotspots/git_file_age. Useful for spotting dead periods, release-crunch spikes, or bus-factor concerns (one author dominating recent history). Returns { ref, sinceDays, totalCommits, activeDays, daysInWindow, byDay: [{date, count}], byAuthor: [{author, count, percentage}], truncatedAuthors }.",
    inputSchema: { type: "object", properties: {
      path:         { type: "string", description: "Any path inside the git repository (used to locate the repo). Defaults to first root." },
      since_days:   { type: "number", description: "Lookback window in days (1–3650, default 30)." },
      ref:          { type: "string", description: "Ref to walk (default 'HEAD')." },
      top_authors:  { type: "number", description: "Max authors in the byAuthor breakdown (1–200, default 10)." },
    }},
  },
  {
    name: "git_orphaned_branches",
    description: "List local branches fully merged into a base branch (safe-to-delete candidates), via a single `git for-each-ref --merged` call. Complements find_stale_branches (age-based signal) with a merge-based signal — a recently-touched but fully-merged branch is still safe to delete, unlike a stale branch that might still carry unmerged work. The current branch and the base branch itself are always excluded from results. Auto-detects 'main' or 'master' as the base when not given; throws a clear error if neither exists and no base is passed. Returns { base, currentBranch, totalBranches, orphanedCount, orphaned: [{name, lastCommitHash, lastCommitShortHash, lastCommitDate, lastCommitSubject, lastCommitAuthor, ageDays}] }.",
    inputSchema: { type: "object", properties: {
      path: { type: "string", description: "Any path inside the git repository (used to locate the repo). Defaults to first root." },
      base: { type: "string", description: "Base branch to check merge-status against (default: auto-detect 'main' or 'master')." },
    }},
  },
  {
    name: "git_submodule_status",
    description: "Report configured vs initialized vs out-of-sync git submodules. Reads `.gitmodules` (name/path/url/branch per configured submodule, via a purpose-built minimal INI-style parser) and cross-references it against `git submodule status`'s per-char status flags: ' ' in-sync (initialized, checked out at the superproject's recorded commit), '-' not-initialized, '+' diverged (checked-out commit differs from the recorded one), 'U' conflicted (merge conflicts in the submodule). A repo with no `.gitmodules` returns hasGitmodules:false and empty arrays, not an error. Returns { hasGitmodules, configuredCount, configured: [{name, path, url, branch}], statusCount, submodules: [{path, sha, status, statusMeaning, describe}], outOfSyncCount }. Always available — does not require MCP_ALLOW_EXEC.",
    inputSchema: { type: "object", properties: {
      path: { type: "string", description: "Any path inside the git repository (used to locate the repo). Defaults to first root." },
    }},
  },
