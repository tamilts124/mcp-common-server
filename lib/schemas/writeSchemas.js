...
const WRITE_SCHEMAS = [
  ...
  {
    name: "extract_tar",
    description: "Extract a .tar/.tar.gz/.tgz file's contents into a directory inside the jail (gzip is auto-detected from the file's magic bytes, not the extension). Companion to create_tar. Validates every entry name up front and rejects symlink/hardlink/device/fifo entries outright (only regular files and directories are supported). The destination directory is created automatically if it does not exist. Pass overwrite: true to extract into an already-existing destination. Write-gated: blocked when MCP_READ_ONLY=true.",
    inputSchema: { type: "object", required: ["path", "destination"], properties: {
      path:        { type: "string",  description: "Client path to the source .tar/.tar.gz/.tgz file." },
      destination: { type: "string",  description: "Client path of the destination directory to extract into. Created automatically if it does not exist." },
      overwrite:   { type: "boolean", description: "If true, extract into an existing directory (colliding files overwritten). Default: false — aborts if destination already exists." },
    }},
  },
  ...
  {
    name: "json_merge",
    description: "Apply an RFC 7396 JSON Merge Patch (a 'patch' JSON document, supplied inline as a string) onto a 'base' JSON file on disk. This is the standard 'merge patch' semantics used by kubectl patch --type merge and HTTP PATCH with application/merge-patch+json: objects merge recursively (patch keys override base keys, keys present only in base are preserved), a null value in the patch DELETES that key from the result (this is the key difference from yaml_merge, where null replaces rather than deletes), and any non-object patch value (including arrays) fully replaces the corresponding base value. Complements json_patch (RFC 6902 explicit ops-list) and yaml_merge (YAML deep-merge). The base file is parsed, merged in memory, re-serialised, and written back. Pass apply:false for a dry-run that returns the merged document without touching the file. Returns { path, apply, originalSize, newSize, result }. Write-gated: blocked when MCP_READ_ONLY=true.",
    inputSchema: { type: "object", required: ["path", "patch"], properties: {
      path:   { type: "string",  description: "Path to the base JSON file to merge onto." },
      patch:  { type: "string",  description: "JSON Merge Patch document text (RFC 7396) to apply onto the base file's contents (e.g. '{\"replicas\":3,\"service\":{\"port\":8080},\"deprecated\":null}')." },
      indent: { type: "number",  description: "Indent width for the written JSON file (default: 2, clamped to 0-8)." },
      apply:  { type: "boolean", description: "If false, perform a dry-run: return the merged document without writing the file (default: true)." },
    }},
  },
  ...
  {
    name: "md_to_pdf",
    description: "Convert a Markdown source file into a PDF file (zero-dependency, hand-built multi-page PDF using the standard-14 Helvetica/Helvetica-Bold fonts, no font embedding). Supports headings (# .. ######, size-scaled), bullet list items (- / *), inline **bold**/*italic* formatting, GFM-style tables (| a | b | with a |---|---| separator row, header row bold, cells pipe-delimited), horizontal rules (---/***/___), word-wrap, and automatic page breaks. Not a full CommonMark engine; ASCII/Latin-1 text only. Write-gated: blocked when MCP_READ_ONLY=true.",
    inputSchema: { type: "object", required: ["path", "destination"], properties: {
      path:        { type: "string", description: "Path to the source Markdown (.md) file." },
      destination: { type: "string", description: "Path where the .pdf file will be written. Parent directories are created automatically." },
    }},
  },
  {
    name: "pdf_to_md",
    description: "Convert a PDF file into a best-effort Markdown/plain-text file (zero-dependency: regex-scans stream/endstream blocks, zlib-inflates FlateDecode-compressed streams, and extracts text from Tj/TJ text-show operators inside BT/ET blocks). No xref-table parsing, no font/glyph-width analysis, so heading/formatting structure is not recovered — output is plain extracted text, one line per text-positioning operator. Works best on text-based PDFs (including ones produced by this server's own md_to_pdf); scanned/image-only PDFs yield no text. Write-gated: blocked when MCP_READ_ONLY=true.",
    inputSchema: { type: "object", required: ["path", "destination"], properties: {
      path:        { type: "string", description: "Path to the source .pdf file." },
      destination: { type: "string", description: "Path where the extracted .md file will be written. Parent directories are created automatically." },
    }},
  },
  {
    name: "docx_to_pdf",
    description: "Convert a Word .docx file directly into a PDF file, preserving inline images (zero-dependency: reads word/document.xml for text/headings/bullets and embeds each referenced picture from word/media/ as a real PDF Image XObject, instead of dropping images the way the docx_to_md -> md_to_pdf round-trip does). JPEG images are embedded byte-for-byte via /DCTDecode (no quality loss). PNG images are hand-decoded (zlib-inflate + per-scanline unfilter) and re-encoded as /FlateDecode DeviceRGB, with alpha channels written as a PDF /SMask. GIF/BMP/TIFF/WMF/EMF/WebP images have no lightweight embedding path here and are rendered as a small italic placeholder line instead of failing the conversion. Not a full OOXML layout engine — tables, columns, and precise spacing are not reproduced, only text flow, headings, bullets, bold/italic, and inline images. Write-gated: blocked when MCP_READ_ONLY=true.",
    inputSchema: { type: "object", required: ["path", "destination"], properties: {
      path:        { type: "string", description: "Path to the source .docx file." },
      destination: { type: "string", description: "Path where the .pdf file will be written. Parent directories are created automatically." },
    }},
  ...
  {
    name: "pdf_to_docx",
    description: "Convert a .pdf file into a best-effort Word .docx file (zero-dependency: same regex-based stream-scan/inflate/Tj-TJ text extraction as pdf_to_md, written out through the same minimal OOXML zip writer as md_to_docx). Output is one plain paragraph per extracted text-positioning line -- no headings, bold/italic, bullets, tables, or images are recovered (this direction has no image extraction path in this codebase; for image-preserving conversion see docx_to_pdf, which goes the other way). Works best on text-based PDFs (including ones produced by this server's own md_to_pdf/docx_to_pdf); scanned/image-only PDFs yield an empty-ish document. Write-gated: blocked when MCP_READ_ONLY=true.",
    inputSchema: { type: "object", required: ["path", "destination"], properties: {
      path:        { type: "string", description: "Path to the source .pdf file." },
      destination: { type: "string", description: "Path where the .docx file will be written. Parent directories are created automatically." },
    }},
  ...
  {
    name: "http_download",
    description: "GET a URL (http/https only) and stream the response body directly to a jailed disk path, following up to 5 redirects. Unlike http_fetch (which buffers the whole body in memory and truncates at 100KB, returning it as UTF-8 text), http_download writes straight to disk as it arrives, so it's the right tool for larger and/or binary payloads: installers, archives, datasets, images. A byte cap (max_bytes, default 100MB, hard-capped at 500MB) is enforced mid-stream — if the response exceeds it, the request is aborted and the partial file is deleted, so callers never end up with a silently-truncated file. GET only (use http_fetch for POST/PUT/etc against APIs). Returns { url, path, status, contentType, bytesWritten, redirected, truncated }. Write-gated: blocked when MCP_READ_ONLY=true.",
    inputSchema: { type: "object", required: ["url", "destination"], properties: {
      url:         { type: "string",  description: "URL to download (http:// or https:// only)." },
      destination: { type: "string",  description: "Path where the downloaded file will be written. Parent directories are created automatically." },
      headers:     { type: "object",  description: "Optional extra request headers, e.g. { Authorization: 'Bearer ...' }." },
      timeout:     { type: "number",  description: "Request timeout in seconds (default: 30, clamped to 1-120)." },
      max_bytes:   { type: "number",  description: "Maximum response size in bytes before the download is aborted (default: 100MB, hard-capped at 500MB)." },
    }},
  },
  {
    name: "json_path_set",
    description: "Mutate a JSON or YAML file by JSONPath query — the write companion to the read-only query_path tool. Supports key access ('.foo' / \\\"['foo']\\\"), array index ('[N]', including one-past-the-end to append), and wildcard ('[*]'/'.*') as the path's steps. Array slice ('[a:b]') and recursive descent ('..') are rejected with a validation error since they're ambiguous as a mutation target — use an explicit key/index/wildcard path instead (query_path remains the tool for that kind of read-only exploration). Two operations: by default the matched location(s) are set to 'value' (JSON text, parsed and cloned fresh per match so multiple matches never alias the same object); pass delete:true to remove the matched key/index instead (array deletions across the same array are applied highest-index-first so earlier removals never shift a later index). The file is parsed, mutated in memory, re-serialised, and written back — pass apply:false for a dry-run. Comments and original YAML formatting are not preserved. Returns { path, query, format, operation, matchCount, apply, originalSize, newSize, result }. Write-gated: blocked when MCP_READ_ONLY=true.",
    inputSchema: { type: "object", required: ["path", "query"], properties: {
      path:   { type: "string",  description: "Path to the JSON or YAML file to mutate." },
      query:  { type: "string",  description: "JSONPath expression identifying the location(s) to set/delete, e.g. '$.spec.replicas', '$.items[2].name', '$.tags[*]'." },
      value:  { type: "string",  description: "JSON text for the value to set at each matched location. Required unless 'delete' is true." },
      delete: { type: "boolean", description: "Delete the matched key/index instead of setting a value (default: false)." },
      format: { type: "string",  description: "Force the file format instead of auto-detecting by extension: 'json' or 'yaml'." },
      indent: { type: "number",  description: "Indent width for JSON output (default: 2, clamped 0-8). Ignored for YAML." },
      apply:  { type: "boolean", description: "If false, perform a dry-run: return the mutated document without writing the file (default: true)." },
    }},
  },
