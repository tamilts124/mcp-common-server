...
const UTIL_SCHEMAS_4 = [
  ...
  {
    name: 'find_memory_leak_patterns',
    description: 'Scans JS/TS files for common patterns that cause unbounded memory growth in long-running Node.js servers. Three rules: module_scope_cache_no_eviction (warning) — a Map or Set declared at module scope (zero indentation) has .set()/.add() calls but no .delete()/.clear() anywhere in the file, so the cache grows without bound; dom_ref_in_module_scope (warning) — a module-scope variable is assigned a document.querySelector*/getElementById* result, preventing GC of the DOM element after removal (browser) or throwing at load time in SSR/Node.js (document is undefined); accumulating_push_in_closure (warning) — a module-scope array has .push() called inside a nested function/callback context but no drain operation (.splice()/.shift()/.pop()/length=0) anywhere in the file, so it grows indefinitely as the callback fires. Siblings: find_setinterval_without_clear, find_missing_remove_event_listener, find_unbounded_object_growth. Pure text-scan (regex, module-scope detection via zero-indentation const/let/var pattern), not an AST/scope parser: the module-scope heuristic matches any declaration whose keyword starts at column 0; an identifier cleared in a separately-imported module is not tracked cross-file. Returns { path, filesScanned, findingsCount, errorCount, warningCount, truncated, findings: [{file,line,variable,rule,severity,message}] }. Always available — does not require MCP_ALLOW_EXEC.',
    inputSchema: { type: 'object', properties: {
      path:       { type: 'string', description: "File or directory to scan (default: '.')." },
      extensions: { type: 'array', items: { type: 'string' }, description: 'File extensions to scan (default: .js/.jsx/.ts/.tsx/.mjs/.cjs).' },
      max_results:{ type: 'number', description: 'Cap on findings[] list length (1-5000, default 500).' },
  ...
  {
    name: 'find_circular_reference_risks',
    description: 'Scans JS/TS files for patterns that create circular object references — structures that cause JSON.stringify() to throw ("Converting circular structure to JSON") and can prevent GC in older engines. Three rules: self_reference_assignment (error) — `obj.prop = obj` / `obj[key] = obj` / `obj.p.q = obj`, an object\u2019s own property is set to itself, the most common accidental circular reference; mutual_module_scope_reference (warning) — two module-scope identifiers (zero-indentation const/let/var) reference each other (`A.ref = B` AND `B.ref = A` in the same file), creating a circular-singleton pair that breaks JSON serialization of either; circular_require_risk (warning) — a require() result is assigned to a name that this file also exports via module.exports/exports, suggesting a likely mutual-require cycle where Node.js would return an incomplete module object at require-time. Siblings: find_memory_leak_patterns, find_circular_deps (import-graph-level cycle detection), find_unbounded_object_growth. Pure text-scan (regex), not an AST/module-graph parser: self-reference detection uses identifier-equality in a single assignment expression; mutual-reference detection is same-file only; circular-require heuristic is name-based and produces false positives when an exported and imported name coincide by coincidence rather than by actual mutual dependency. Returns { path, filesScanned, findingsCount, errorCount, warningCount, truncated, findings: [{file,line,variable?,variables?,required?,rule,severity,message}] }. Always available — does not require MCP_ALLOW_EXEC.',
    inputSchema: { type: 'object', properties: {
      path:       { type: 'string', description: "File or directory to scan (default: '.')." },
      extensions: { type: 'array', items: { type: 'string' }, description: 'File extensions to scan (default: .js/.jsx/.ts/.tsx/.mjs/.cjs).' },
      max_results:{ type: 'number', description: 'Cap on findings[] list length (1-5000, default 500).' },
  ...
  {
    name: 'find_promise_constructor_antipattern',
    description: 'Scans JS/TS files for two common Promise constructor misuses. async_executor_in_promise_constructor (error) — `new Promise(async (resolve, reject) => {...})` uses an async executor; if the executor throws after an await, the error escapes the Promise constructor entirely and becomes an unhandled rejection (the Promise constructor only wraps synchronous throws from the executor, not async ones). Fix: remove the Promise wrapper and make the caller async. explicit_promise_wrap (warning) — `new Promise((resolve, reject) => somePromise.then(resolve, reject))` or `.then(resolve).catch(reject)` wraps an already-thenable value unnecessarily; return the inner promise directly. Siblings: find_dangling_promises, find_missing_try_catch_in_async, find_unhandled_rejection_patterns, find_promise_all_without_catch. Pure text-scan (balanced-paren executor extraction + regex), not an AST parser. Returns { path, filesScanned, findingsCount, errorCount, warningCount, truncated, findings: [{file,line,rule,severity,message}] }. Always available — does not require MCP_ALLOW_EXEC.',
    inputSchema: { type: 'object', properties: {
      path:       { type: 'string', description: "File or directory to scan (default: '.')." },
      extensions: { type: 'array', items: { type: 'string' }, description: 'File extensions to scan (default: .js/.jsx/.ts/.tsx/.mjs/.cjs).' },
      max_results:{ type: 'number', description: 'Cap on findings[] list length (1-5000, default 500).' },
  ...
  {
    name: 'find_event_emitter_leak',
    description: 'Scans JS/TS files for EventEmitter listener accumulation patterns that cause Node.js MaxListenersExceededWarning and silent memory leaks. Two rules: process_listener_in_function_body (error) — `process.on(event, handler)` called inside a function body (indented line), not at module scope; every call permanently adds a new listener to the global process object, and after >10 calls Node.js emits MaxListenersExceededWarning; common offender is adding uncaughtException/unhandledRejection handlers inside middleware or request handlers. emitter_on_inside_loop (warning) — `.on(event, handler)` or `.once(event, handler)` inside a for/while/do loop body (detected via a 15-line lookback for a loop header); each iteration attaches a new listener without removing the previous, growing the listener list with loop iterations. Siblings: find_missing_remove_event_listener, find_setinterval_without_clear, find_memory_leak_patterns. Pure text-scan (regex + indentation heuristic + loop-header lookback), not an AST/scope parser. Returns { path, filesScanned, findingsCount, errorCount, warningCount, truncated, findings: [{file,line,event,emitter?,rule,severity,message}] }. Always available — does not require MCP_ALLOW_EXEC.',
    inputSchema: { type: 'object', properties: {
      path:       { type: 'string', description: "File or directory to scan (default: '.')." },
      extensions: { type: 'array', items: { type: 'string' }, description: 'File extensions to scan (default: .js/.jsx/.ts/.tsx/.mjs/.cjs).' },
      max_results:{ type: 'number', description: 'Cap on findings[] list length (1-5000, default 500).' },
  ...
  {
    name: 'find_sql_injection_risk',
    description: 'Scans JS/TS files for SQL injection risks. Three rules: sql_string_concat (error) - SQL keyword string concatenated with user-controlled input (req.body/query/params or sensitive-named vars like id/name/user/email/input) without parameterized-query hint; sql_template_literal (error) - template literal with SQL keyword interpolating user-controlled identifiers; sql_dynamic_query_variable (warning) - variable named *Sql/*Query/*Statement built by += concatenation. Suppressions: same-line // safe or // nosql annotation, or parameterized-query hint (.query(sql, [), .prepare(), stmt.run()). Returns { path, filesScanned, findingsCount, errorCount, warningCount, truncated, findings: [{file,line,rule,severity,message}] }. Always available - does not require MCP_ALLOW_EXEC.',
    inputSchema: { type: 'object', properties: {
      path:       { type: 'string', description: "File or directory to scan (default: '.')." },
      extensions: { type: 'array', items: { type: 'string' }, description: 'File extensions to scan (default: .js/.jsx/.ts/.tsx/.mjs/.cjs).' },
      max_results:{ type: 'number', description: 'Cap on findings[] list length (1-5000, default 500).' },
  ...
  {
    name: 'find_command_injection_risk',
    description: 'Scans JS/TS files for command injection risks: exec/execSync/spawn/spawnSync/execFile/execFileSync calls that build their command argument by string concatenation or template-literal interpolation using user-controlled values. Three rules: exec_with_concat (error) - dynamic argument containing user input (req.* or sensitive identifiers: input/param/value/data/arg/cmd/command/file/path); shell_true_spawn_with_input (error) - spawn/spawnSync with { shell: true } and a dynamic argument (shell:true passes command to /bin/sh -c making injection trivial via ; | metacharacters); unvalidated_path_exec (warning) - exec with path-like variable and no visible path.resolve/normalize/allowlist check. Suppressions: same-line // safe, // noexec, // no-exec. Returns { path, filesScanned, findingsCount, errorCount, warningCount, truncated, findings: [{file,line,rule,severity,message}] }. Always available - does not require MCP_ALLOW_EXEC.',
    inputSchema: { type: 'object', properties: {
      path:       { type: 'string', description: "File or directory to scan (default: '.')." },
      extensions: { type: 'array', items: { type: 'string' }, description: 'File extensions to scan (default: .js/.jsx/.ts/.tsx/.mjs/.cjs).' },
      max_results:{ type: 'number', description: 'Cap on findings[] list length (1-5000, default 500).' },
  ...
  {
    name: 'find_xss_risk',
    description: 'Scans JS/TS/JSX/TSX files for Cross-Site Scripting (XSS) sinks where user-controlled values are assigned without sanitization. Three rules: inner_html_assignment (error) — .innerHTML= or .outerHTML= assigned a value tainted by req.*, event.target.value, location.*, URLSearchParams, document.cookie, or sensitive-name identifiers (input/value/query/user/content/html/markup/text) without a DOMPurify.sanitize/escapeHtml/he.escape call; document_write_with_input (error) — document.write()/document.writeln() with a template-literal or concatenated argument containing user-tainted values; insert_adjacent_html_with_input (error) — insertAdjacentHTML() second argument contains user-tainted values. Suppressions: same-line // safe, // xss-safe, // sanitized, or a sanitization hint (DOMPurify.sanitize, escapeHtml, he.escape, htmlspecialchars, sanitize, encode). Pure text-scan (regex), not an AST/data-flow parser. Returns { path, filesScanned, findingsCount, errorCount, warningCount, truncated, findings: [{file,line,rule,severity,message}] }. Always available — does not require MCP_ALLOW_EXEC.',
    inputSchema: { type: 'object', properties: {
      path:       { type: 'string', description: "File or directory to scan (default: '.')." },
      extensions: { type: 'array', items: { type: 'string' }, description: 'File extensions to scan (default: .js/.jsx/.ts/.tsx/.mjs/.cjs).' },
      max_results:{ type: 'number', description: 'Cap on findings[] list length (1-5000, default 500).' },
  ...
  {
    name: 'find_path_traversal_risk',
    description: 'Scans JS/TS files for path traversal vulnerabilities where user-controlled input flows into file system operations without adequate validation. Three rules: path_join_with_user_input (error) — path.join()/path.resolve()/path.normalize() called with user-controlled input (req.body/query/params/headers or path-like identifier names: file/dir/path/filename/filepath/folder/directory) without a visible containment guard (startsWith, path.relative, allowlist, ALLOW*); fs_readwrite_with_user_input (error) — fs.readFile/writeFile/appendFile/readFileSync/writeFileSync/createReadStream/createWriteStream/unlink/rename with user-tainted first argument and no validation guard; send_file_with_user_input (warning) — Express res.sendFile()/res.download() with user-controlled path argument. Suppressions: same-line // safe or // path-safe, or guard presence (startsWith, path.relative, allowlist). Pure text-scan (regex), not an AST/data-flow parser. Returns { path, filesScanned, findingsCount, errorCount, warningCount, truncated, findings: [{file,line,rule,severity,message}] }. Always available — does not require MCP_ALLOW_EXEC.',
    inputSchema: { type: 'object', properties: {
      path:       { type: 'string', description: "File or directory to scan (default: '.')." },
      extensions: { type: 'array', items: { type: 'string' }, description: 'File extensions to scan (default: .js/.jsx/.ts/.tsx/.mjs/.cjs).' },
      max_results:{ type: 'number', description: 'Cap on findings[] list length (1-5000, default 500).' },
  ...
  {
    name: 'find_insecure_deserialization',
    description: 'Scans JS/TS files for insecure deserialization patterns enabling arbitrary code execution or prototype pollution. Three rules: unsafe_deserializer_call (error) \u2014 node-serialize unserialize(), vm.runInThisContext/InContext/InNewContext(), or dynamic eval wrappers (nodeEval/dynamicRequire) which can execute arbitrary JavaScript from deserialized data; unsafe_yaml_load (error) \u2014 yaml.load()/YAML.parse() without { schema: SAFE_SCHEMA } allowing arbitrary JS-object deserialization via YAML type tags (!!js/undefined etc.); json_parse_untrusted_source (warning) \u2014 JSON.parse() applied directly to req.body/query/params/headers, socket data, or fs.readFile results without visible schema validation, risking SyntaxError crashes and type-confusion bugs. Suppressions: same-line // safe, // trusted, // sanitized, // nosec. Returns { path, filesScanned, findingsCount, errorCount, warningCount, truncated, findings: [{file,line,rule,severity,message}] }. Always available \u2014 does not require MCP_ALLOW_EXEC.',
    inputSchema: { type: 'object', properties: {
      path:       { type: 'string', description: "File or directory to scan (default: '.')." },
      extensions: { type: 'array', items: { type: 'string' }, description: 'File extensions to scan (default: .js/.jsx/.ts/.tsx/.mjs/.cjs).' },
      max_results:{ type: 'number', description: 'Cap on findings[] list length (1-5000, default 500).' },
  ...
  {
    name: 'find_prototype_pollution_via_merge',
    description: 'Scans JS/TS files for prototype pollution vectors where deep-merge or recursive object-assign operations are performed on user-controlled input without sanitizing __proto__, constructor, or prototype keys. Three rules: deep_merge_with_user_input (error) \u2014 _.merge/lodash.merge/deepmerge/deepMerge/jQuery.extend(true) called with req.body/query/params/headers or body/input/data/payload identifiers and no __proto__/constructor/prototype sanitization guard in a \xb15-line window; recursive_assign_with_user_input (error) \u2014 custom mergeDeep/deepAssign/deepExtend/recursiveMerge/extend function called with user-controlled input and no sanitization; object_assign_spread_no_sanitize (warning) \u2014 Object.assign({}, req.*) or spread {...req.*} without key sanitization (shallow merge, lower severity). Suppressions: same-line // safe, // sanitized, // no-pollution, // nosec, or presence of __proto__/hasOwnProperty/Object.create(null)/sanitize in a \xb15-line window. Returns { path, filesScanned, findingsCount, errorCount, warningCount, truncated, findings: [{file,line,rule,severity,message}] }. Always available \u2014 does not require MCP_ALLOW_EXEC.',
    inputSchema: { type: 'object', properties: {
      path:       { type: 'string', description: "File or directory to scan (default: '.')." },
      extensions: { type: 'array', items: { type: 'string' }, description: 'File extensions to scan (default: .js/.jsx/.ts/.tsx/.mjs/.cjs).' },
      max_results:{ type: 'number', description: 'Cap on findings[] list length (1-5000, default 500).' },
  ...
  {
    name: 'find_timing_attack_risk',
    description: 'Scans JS/TS files for timing-attack vulnerabilities where security-sensitive values (passwords, tokens, secrets, hashes, HMAC digests, API keys, signatures) are compared using `===` or `!==` instead of the constant-time `crypto.timingSafeEqual()`. Variable-time comparison leaks secret length and content through timing side channels — an attacker can determine secret content character-by-character by measuring response times. Two rules: unsafe_secret_comparison (error) — an equality comparison (`===`/`!==`/`==`/`!=`) involving a variable whose name matches a security-sensitive pattern (password/passwd/pwd/secret/token/accessToken/refreshToken/hash/hmac/digest/signature/sig/apiKey/api_key/sessionId/authCode/authToken/otp/nonce) where at least one side is not a hardcoded string literal; string_equality_on_hash (warning) — a Buffer.from() call or a hex-encoded string literal (>=16 hex chars) compared with `===`. Suppressions: same-line `// safe` or `// timing-safe` annotation, or presence of `timingSafeEqual`/`bcrypt.compare`/`argon2.verify`/`scryptSync`/`pbkdf2` anywhere in a ±5-line window. Fix: use `crypto.timingSafeEqual(Buffer.from(a), Buffer.from(b))`. Returns { path, filesScanned, findingsCount, errorCount, warningCount, truncated, findings: [{file,line,rule,severity,message}] }. Always available — does not require MCP_ALLOW_EXEC.',
    inputSchema: { type: 'object', properties: {
      path:       { type: 'string', description: "File or directory to scan (default: '.')." },
      extensions: { type: 'array', items: { type: 'string' }, description: 'File extensions to scan (default: .js/.jsx/.ts/.tsx/.mjs/.cjs).' },
      max_results:{ type: 'number', description: 'Cap on findings[] list length (1-5000, default 500).' },
  ...
  {
    name: 'find_missing_input_validation',
    description: 'Scans JS/TS files for Express routes that access req.body, req.query, or req.params without a visible validation middleware guard. Unvalidated external input is the root cause of injection attacks, type confusion bugs, and unexpected server crashes. Two rules: route_body_no_validation (error) — an Express POST/PUT/PATCH/DELETE route handler that accesses `req.body` but the file has no visible validation library usage (Joi, Zod, express-validator, yup, AJV, celebrate, valibot) and no call to a validate/schema/sanitize helper; route_query_no_validation (warning) — same for GET routes that use `req.query` or `req.params` without validation. File-level suppression: if any of Joi.object/z.object/z.string/body(\')/query(\')/param(\'/check(/ajv.compile/new Ajv/yup.object/validate(/sanitize(/celebrate/valibot/`// validated` appear anywhere in the file, the entire file is considered protected. Per-line suppression: `// safe`, `// no-validate`, or `// validated`. Returns { path, filesScanned, findingsCount, errorCount, warningCount, truncated, findings: [{file,line,rule,severity,message}] }. Always available — does not require MCP_ALLOW_EXEC.',
    inputSchema: { type: 'object', properties: {
      path:       { type: 'string', description: "File or directory to scan (default: '.')." },
      extensions: { type: 'array', items: { type: 'string' }, description: 'File extensions to scan (default: .js/.jsx/.ts/.tsx/.mjs/.cjs).' },
      max_results:{ type: 'number', description: 'Cap on findings[] list length (1-5000, default 500).' },
  ...
  {
    name: 'find_race_condition_risk',
    description: 'Scans JS/TS files for non-atomic read-then-write sequences on shared module-scope mutable state inside async functions, Promise callbacks, and setTimeout/setInterval handlers. Three rules: non_atomic_readwrite_in_async (error) — a module-scope let/var is read then written within 10 lines inside an async context with no mutex/lock/semaphore guard visible in a ±8-line window; check_then_act_race (error) — an `if (sharedVar)` guard is followed within 12 lines by a mutation of that same var inside an async context (TOCTOU: time-of-check / time-of-use), no lock guard; shared_counter_no_lock (warning) — a module-scope variable whose name matches count/counter/seq/index/idx/hits/request/connection/pending is incremented (++/+= N) inside an indented function body without a lock guard. Suppressions: same-line // safe, // atomic, // no-race, // nosec, or presence of mutex/lock/semaphore/Semaphore/Mutex/AsyncLock/async-lock/p-queue/p-limit/sequelize.transaction in a ±8-line window. Pure text-scan (regex + indentation heuristics + async-context lookback), not an AST/data-flow parser: module-scope detection uses zero-indentation let/var heuristic; async-context detection is a 30-line brace-depth walkback for async/setTimeout/setInterval/then/catch/new Promise headers — cross-function tracking not performed. Returns { path, filesScanned, findingsCount, errorCount, warningCount, truncated, findings: [{file,line,variable,rule,severity,message}] }. Always available — does not require MCP_ALLOW_EXEC.',
    inputSchema: { type: 'object', properties: {
      path:       { type: 'string', description: "File or directory to scan (default: '.')." },
      extensions: { type: 'array', items: { type: 'string' }, description: 'File extensions to scan (default: .js/.jsx/.ts/.tsx/.mjs/.cjs).' },
      max_results:{ type: 'number', description: 'Cap on findings[] list length (1-5000, default 500).' },
  ...
  {
    name: 'find_unvalidated_redirect',
    description: 'Scans JS/TS files for redirect operations using dynamic (non-literal) values without an allowlist or origin check — broader than find_open_redirect_risks (which only flags DIRECT req.* in the redirect call); this tool flags ANY dynamic variable or expression in a redirect sink without a visible guard. Four rules: redirect_dynamic_url (error) — res.redirect(expr) where expr is a template literal, variable, or concatenation with no startsWith/includes/allowlist/origin/pathname guard in a ±5-line window; location_href_dynamic (error) — window.location.href = expr / location.href = expr / window.location = expr with a dynamic RHS and no validation guard; location_header_dynamic (warning) — res.setHeader(\'Location\', dynamic) / res.writeHead(3xx, {\'Location\': dynamic}) without a guard; next_with_dynamic_route (warning) — next(varName) where varName matches route/url/redirect/dest/path naming patterns (tainted next() call can skip auth middleware). Suppressions: same-line // safe, // validated, // redirect-safe, // nosec, or presence of startsWith/includes/allowlist/allow_list/whitelist/ALLOWED/origin ===/pathname ===/url.parse/new URL( in a ±5-line window. Pure text-scan (regex + balanced-paren arg extraction), not an AST/data-flow parser: dynamic detection uses template-literal interpolation, identifier, or concatenation heuristics; string literals and backtick strings without ${ } are treated as safe. Returns { path, filesScanned, findingsCount, errorCount, warningCount, truncated, findings: [{file,line,rule,severity,message}] }. Always available — does not require MCP_ALLOW_EXEC.',
    inputSchema: { type: 'object', properties: {
      path:       { type: 'string', description: "File or directory to scan (default: '.')." },
      extensions: { type: 'array', items: { type: 'string' }, description: 'File extensions to scan (default: .js/.jsx/.ts/.tsx/.mjs/.cjs).' },
      max_results:{ type: 'number', description: 'Cap on findings[] list length (1-5000, default 500).' },
