...
const UTIL_SCHEMAS = [
  ...
  {
    name: "csv_query",
    description: "Parse a CSV file (RFC 4180 — handles quoted fields, embedded commas and newlines, optional BOM) and return rows as structured JSON objects. Supports column projection (select a subset of columns), row slicing (offset + limit), and a simple single-column equality filter. The first row is treated as a header by default (set has_header: false for header-less files). Returns: path, columns, totalRows (after filtering), returnedRows, and rows array. Always available — does not require MCP_ALLOW_EXEC.",
    inputSchema: { type: "object", required: ["path"], properties: {
      path:       { type: "string",  description: "Path to the CSV file to query." },
      columns:    { type: "array",   items: { type: "string" }, description: "Column names to include in output (default: all)." },
      offset:     { type: "number",  description: "Skip the first N data rows (default: 0)." },
      limit:      { type: "number",  description: "Maximum number of rows to return (1–10000, default: 100)." },
      filter_col: { type: "string",  description: "Column name to filter on (exact equality, case-sensitive)." },
