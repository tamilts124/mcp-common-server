...
const SESSION_SCHEMAS = [
  {
    ...
    inputSchema: { type: "object", properties: {
      ...
      proxy: {
        type: "object",
        description: "Optional proxy configuration. Supports HTTP, HTTPS, SOCKS4, SOCKS5. For socks5:// proxies, DNS is automatically routed through the tunnel (socks5_dns: true by default) which is required for .onion / Tor hidden services.",
        properties: {
          server:     { type: "string",  description: "Proxy server URL, e.g. 'socks5://127.0.0.1:9050', 'http://proxy.example.com:8080'." },
          username:   { type: "string",  description: "Optional proxy username." },
          password:   { type: "string",  description: "Optional proxy password." },
          bypass:     { type: "string",  description: "Optional comma-separated list of hosts to bypass the proxy for, e.g. 'localhost,127.0.0.1'." },
          socks5_dns: { type: "boolean", description: "For socks5:// proxies: route DNS through the SOCKS5 tunnel (default: true). Required for .onion domains. Set false to disable." },
        },
        required: ["server"],
      },
      chromium_args: {
        type: "array",
        items: { type: "string" },
        description: "Optional list of extra Chromium CLI flags to pass at launch, e.g. ['--disable-web-security']. Only flags starting with '--' are accepted.",
      },
      ignore_https_errors: { type: "boolean", description: "Ignore HTTPS/TLS certificate errors (default: false). Useful for local dev servers or onion sites with self-signed certs." },
    }},
  },
  {
