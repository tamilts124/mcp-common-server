...
const { CMD_TIMEOUT, CMD_TIMEOUT_MAX } = require("../config");
...
const EXEC_SCHEMAS = [
  {
    ...
    inputSchema: { type: "object", required: ["command"], properties: {
      ...
      timeout: { type: "number",  description: `Seconds before the command is killed (default: ${CMD_TIMEOUT}, max: ${CMD_TIMEOUT_MAX}).` },
  ...
  },
  {
    ...
    description: "Send text (or raw bytes) to the stdin of a background process started with start_process. Enables interactive process communication: type commands into a REPL, answer a CLI prompt, control a database shell, or drive any program that reads from stdin. A trailing newline is appended by default (the common 'press Enter' behaviour for CLIs) — pass add_newline:false to suppress it (useful for raw byte streams or protocols that don't use line-oriented input). Data can be plain UTF-8 text or base64-encoded bytes (set encoding:'base64'). Requires the process to be still running. Returns { id, pid, bytesWritten, addedNewline, status }. Requires MCP_ALLOW_EXEC=true.",
    inputSchema: { type: "object", required: ["id", "data"], properties: {
      id:          { type: "string",  description: "Process id returned by start_process." },
      data:        { type: "string",  description: "Text or bytes to write to the process's stdin. UTF-8 by default; pass encoding:'base64' for raw bytes." },
      encoding:    { type: "string",  enum: ["utf8", "base64"], description: "Encoding of the 'data' field (default: 'utf8')." },
      add_newline: { type: "boolean", description: "Append a '\\n' after the data (default: true). Set false for raw/binary protocols." },
    }},
  },
  {
    name: "run_npm_script",
    ...
    inputSchema: { type: "object", required: ["script"], properties: {
      ...
      timeout:    { type: "number", description: `Seconds before the script is killed (default: ${CMD_TIMEOUT}, max: ${CMD_TIMEOUT_MAX}).` },
      env:        { type: "object", description: "Extra environment variables to merge in.",
        additionalProperties: { type: "string" },
      },
    }},
...

  {
    name: "git_write_ops",
    description:
      "Perform git write operations (add, commit, push, pull, checkout, branch, reset, stash, " +
      "merge, rebase, cherry_pick, tag) on a repository inside a configured root. " +
      "Uses spawnSync with an args array (no shell) so commit messages, tag names, branch names, " +
      "and remote URLs cannot cause shell injection. Requires MCP_ALLOW_EXEC=true.",
    inputSchema: {
      type: "object",
      required: ["operation"],
      properties: {
        operation: {
          type: "string",
          enum: ["add", "commit", "push", "pull", "checkout", "branch", "reset", "stash", "merge", "rebase", "cherry_pick", "tag"],
          description: "Git operation to perform.",
        },
        path: { type: "string", description: "Path inside a configured root to locate the git repository (default: first root)." },
        paths: { type: "array", items: { type: "string" }, description: "add/reset: specific file paths to stage/unstage. Omit to stage all." },
        all: { type: "boolean", description: "add: stage all changes (default: true when paths not provided)." },
        message: { type: "string", description: "commit/merge/stash: commit message or stash description (max 10000 chars)." },
        author: { type: "string", description: "commit: override author in 'Name <email>' format." },
        amend: { type: "boolean", description: "commit: amend the previous commit." },
        no_edit: { type: "boolean", description: "commit: keep existing message when amending (use with amend:true)." },
        allow_empty: { type: "boolean", description: "commit: allow commit with no staged changes." },
        allow_empty_message: { type: "boolean", description: "commit: allow a blank commit message." },
        no_verify: { type: "boolean", description: "commit: skip pre-commit and commit-msg hooks." },
        remote: { type: "string", description: "push/pull: remote name (default: 'origin')." },
        branch: { type: "string", description: "push/pull/checkout/merge: branch name." },
        force: { type: "boolean", description: "push: force-push. branch delete: force-delete." },
        force_with_lease: { type: "boolean", description: "push: --force-with-lease safety flag." },
        set_upstream: { type: "boolean", description: "push: --set-upstream (-u)." },
        tags: { type: "boolean", description: "push: also push tags." },
        delete_branch: { type: "string", description: "push: delete a remote branch (--delete <name>)." },
        rebase: { type: "boolean", description: "pull: --rebase." },
        ff_only: { type: "boolean", description: "pull: --ff-only." },
        no_commit: { type: "boolean", description: "pull/merge: --no-commit." },
        create: { type: "boolean", description: "checkout: create branch (-b)." },
        from: { type: "string", description: "checkout/branch: start-point ref." },
        action: {
          type: "string",
          enum: ["create", "delete", "rename", "list", "continue", "abort", "skip", "quit"],
          description: "branch: create/delete/rename/list. rebase/cherry_pick: continue/abort/skip/quit.",
        },
        name: { type: "string", description: "branch/tag: branch or tag name." },
        target: { type: "string", description: "branch rename: new name." },
        mode: { type: "string", enum: ["soft", "mixed", "hard"], description: "reset: mode (default: mixed)." },
        ref: { type: "string", description: "reset ref to reset to (default: HEAD). cherry_pick: commit ref to pick. tag: ref to tag." },
        subop: {
          type: "string",
          enum: ["push", "pop", "drop", "list", "show", "clear"],
          description: "stash: sub-operation (default: push).",
        },
        stash: { type: "string", description: "stash pop/drop/show: stash ref e.g. stash@{0}." },
        include_untracked: { type: "boolean", description: "stash push: --include-untracked." },
        no_ff: { type: "boolean", description: "merge: --no-ff (always create merge commit)." },
        squash: { type: "boolean", description: "merge: --squash." },
        abort: { type: "boolean", description: "merge: --abort an in-progress merge." },
        onto: { type: "string", description: "rebase: --onto <newbase>." },
        mainline: { type: "number", description: "cherry_pick: -m <parent-number> for merge commits (positive integer)." },
      },
    },
  },
          ...
          properties: {
            op: {
              ...
              enum: [
                ...
                "kill_process", "list_processes", "send_process_input", "run_npm_script",
