...
const BROWSER_SCHEMAS = [
  ...
  },
  {
    ...
    description: "Snapshot the session context's storage state (cookies + per-origin localStorage) as a portable object. Pass the result as browser_launch's storage_state to resume the same logged-in session in a fresh browser.",
    inputSchema: { type: "object", required: ["session_id"], properties: {
      session_id: { type: "string", description: "Session id returned by browser_launch." },
    }},
  },
  ...
  },
  {
    ...
    description: "Arm a one-shot accept/dismiss action for the next alert/confirm/prompt dialog on this session. Without arming, dialogs are auto-dismissed (Playwright's no-listener default) and logged. Call this before the action that triggers the dialog.",
