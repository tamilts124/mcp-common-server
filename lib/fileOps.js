...
function searchRecursive(dirPath, pattern, isRegex, alias, results = [], warnings = []) {
  const entries = fs.readdirSync(dirPath, { withFileTypes: true });
  for (const e of entries) {
    if (isIgnored(e.name)) continue;
    const full = path.join(dirPath, e.name);
    if (e.isDirectory()) { searchRecursive(full, pattern, isRegex, alias, results, warnings); continue; }
    try {
      ...
      const msg = `[search_files] failed to read/search file: ${full}: ${e.message}`;
      console.error(msg);
      warnings.push(msg);
    }
  }
