...
function executeTool(name, args) {
  ...
  const handler = READ_DISPATCH[name] || WRITE_DISPATCH[name] || BROWSER_DISPATCH[name] || EMAIL_DISPATCH[name] || SQLITE_DISPATCH[name];
...
async function executePipeline(steps) {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    try {
      ...
      // silently overflow the outer pipeline envelope and trigger the proxy
      // "Error occurred during tool execution" generic error.
      const { text, truncated, originalBytes } = serializeResult(rawResult);
      const result = truncated
        ? { __truncated: true, originalBytes, data: text }  // already JSON string
        : rawResult;                                          // keep raw object for JSON-RPC serialiser
      if (truncated) {
      ...
      // Include full error detail (code + message + stack) so the caller can
      // debug without needing to re-run the tool in isolation.
