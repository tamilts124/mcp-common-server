...
const { fileChecksum, zipDirectory, queryJson, queryData } = require("./utilOps");
const { gitStatus, gitLog, gitBlame } = require("./gitOps");
...
// ── JSON-RPC TAGGED ERRORS ──────────────────────────────────────────────────
// Standard JSON-RPC 2.0 codes used here:
...
// -32001 (app-reserved range) is used for "tool forbidden by server policy"
// (read-only mode / exec disabled) since that's neither a malformed request
// nor an unexpected internal failure — it's an explicit, expected refusal.
...
// Returns the JSON-RPC error code to report for any thrown error: the
// error's own .code if tagged, otherwise -32603 (internal error) by default.
