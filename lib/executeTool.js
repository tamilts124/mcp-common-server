...
const { READ_ONLY, ALLOW_EXEC, CATEGORIES } = require("./config");
const { WRITE_TOOLS, EXEC_TOOLS, TOOLS_ALL, filterByCategories } = require("./toolsSchema");
...
const { EMAIL_DISPATCH }     = require("./dispatchEmail");
const { SQLITE_DISPATCH }    = require("./dispatchSqlite");
const { COMMUNITY_DISPATCH } = require("./dispatchCommunity");
const { GH_DISPATCH }        = require("./dispatchGh");
const { injectInboxSummary } = require("./communityOps");
...
// Apply category filter first (--category=community,git etc), then policy gates.
const TOOLS = filterByCategories(CATEGORIES).filter(t => {
...
function executeTool(name, args) {
  ...
  const handler = READ_DISPATCH[name] || WRITE_DISPATCH[name] || BROWSER_DISPATCH[name] || EMAIL_DISPATCH[name] || SQLITE_DISPATCH[name] || COMMUNITY_DISPATCH[name] || GH_DISPATCH[name];
  if (!handler) throw new ToolError(`Unknown tool: ${name}`, -32601);

  // Single central recording hook: if this is a browser_* tool called with a
  ...
  // throws, so a recorded exploration session reflects what was actually attempted.
  if (BROWSER_DISPATCH[name] && args && args.session_id && !RECORDING_EXCLUDED.has(name)) {
    recordAction(args.session_id, name, args);
  }

  // Passive inbox notification: for any tool call that carries a session_id
  // (from ANY category, not just community tools), staple unread/total counts
  // onto the result so the AI never silently misses messages.
  const rawResult = handler(args);
  if (args && args.session_id && !name.startsWith("community_")) {
    // Sync path
    if (rawResult && typeof rawResult.then !== "function") {
      return injectInboxSummary(args.session_id, rawResult);
    }
    // Async path
    if (rawResult && typeof rawResult.then === "function") {
      return rawResult.then(r => injectInboxSummary(args.session_id, r));
    }
  }
...
async function executePipeline(steps) {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    try {
      const rawResult = await Promise.resolve(executeTool(op, toolArgs));
      // Cap each step result at 3.5 MB so a large intermediate result can't
      // silently overflow the outer pipeline envelope.
      const { text, truncated, originalBytes } = serializeResult(rawResult);
      const result = truncated
        ? { __truncated: true, originalBytes, data: text }
        : rawResult;
      if (truncated) {
        console.error(`[PIPELINE] step ${i} '${op}' result truncated (${originalBytes} bytes > 3.5 MB)`);
      }
      results.push({ index: i, op, status: "ok", result });
      completed++;
    } catch (e) {
      const detail = formatError(e, op);
      console.error(`[PIPELINE ERROR] step ${i}: ${detail}`);
      results.push({ index: i, op, status: "error", error: e.message, errorDetail: detail });
      if (onError === "stop") {
        stoppedAt = i;
