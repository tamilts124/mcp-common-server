...
function opAdd(args, repoDir, resolveClientPath) {
  ...
  } catch (e) { console.error("[git_write_ops] failed to get staged file count (before add):", e.message); }
  ...
  try {
    const s = gitSpawn(["diff", "--cached", "--name-only"], repoDir);
    stagedAfter = s.split("\n").filter(Boolean).length;
  } catch (e) { console.error("[git_write_ops] failed to get staged file count (after add):", e.message); }
