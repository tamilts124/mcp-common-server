...
function opAdd(args, repoDir, resolveClientPath) {
  ...
  try {
    ...
    const msg = `[git_write_ops] failed to get staged file count (before add): ${e.message}`;
    console.error(msg);
    _warnings.push(msg);
  }
  ...
  try {
    ...
    const msg = `[git_write_ops] failed to get staged file count (after add): ${e.message}`;
    console.error(msg);
    _warnings.push(msg);
  }
