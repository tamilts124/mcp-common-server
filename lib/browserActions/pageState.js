...
async function goBack(args = {}) {
  ...
  const { page } = getSession(args.session_id);
  try {
    await page.goBack({ timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT), waitUntil: args.wait_until || "load" });
  } catch (e) {
...
async function goForward(args = {}) {
  ...
  const { page } = getSession(args.session_id);
  try {
    await page.goForward({ timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT), waitUntil: args.wait_until || "load" });
  } catch (e) {
...
async function reload(args = {}) {
  ...
  const { page } = getSession(args.session_id);
  try {
    await page.reload({ timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT), waitUntil: args.wait_until || "load" });
  } catch (e) {
...
async function waitForNavigation(args = {}) {
  ...
  const { page } = getSession(args.session_id);
  try {
    await page.waitForLoadState(args.wait_until || "load", { timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) });
  } catch (e) {
