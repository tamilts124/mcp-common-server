...
async function setCookies(args = {}) {
  ...
  // Load cookies from a file if cookies_file is provided (cookies_file takes precedence)
  if (args.cookies_file) {
    if (typeof args.cookies_file !== "string" || args.cookies_file.trim() === "")
      throw new ToolError("browser_set_cookies: 'cookies_file' must be a non-empty string path.", -32602);
    let raw;
    try {
      raw = fs.readFileSync(args.cookies_file, "utf8");
    } catch (e) {
      throw new ToolError(`browser_set_cookies: cannot read cookies_file '${args.cookies_file}': ${e.message}`, -32602);
    }
    try {
      ...
      throw new ToolError(`browser_set_cookies: cookies_file '${args.cookies_file}' is not valid JSON: ${e.message}`, -32602);
    }
  }

  if (!Array.isArray(cookies) || cookies.length === 0)
    throw new ToolError("browser_set_cookies requires a non-empty 'cookies' array (or a valid cookies_file).", -32602);
  ...
  // Normalize cookies from browser-extension format to Playwright format.
  // Browser extensions export sameSite as null or "no_restriction"; Playwright
  // only accepts "Strict", "Lax", or "None". Also maps expirationDate → expires.
  const SAME_SITE_MAP = {
    "no_restriction": "None",
    "lax":            "Lax",
    "strict":         "Strict",
  ...
  const playwrightCookies = cookies.map(c => {
    const n = {};
    if (c.name     !== undefined) n.name     = c.name;
    if (c.value    !== undefined) n.value    = c.value;
    if (c.domain   !== undefined) n.domain   = c.domain;
    if (c.path     !== undefined) n.path     = c.path;
    if (c.url      !== undefined) n.url      = c.url;
    if (c.secure   !== undefined) n.secure   = !!c.secure;
    if (c.httpOnly !== undefined) n.httpOnly = !!c.httpOnly;
    // Chrome extension uses expirationDate (float); Playwright uses expires (int)
    if (c.expires !== undefined)              n.expires = Math.floor(c.expires);
    else if (c.expirationDate !== undefined)  n.expires = Math.floor(c.expirationDate);
    // Normalize sameSite: null / "no_restriction" / unknown → omit (browser default)
    if (c.sameSite !== null && c.sameSite !== undefined) {
      const mapped = SAME_SITE_MAP[String(c.sameSite).toLowerCase()];
      if (mapped)                                 n.sameSite = mapped;
      else if (["Strict","Lax","None"].includes(c.sameSite)) n.sameSite = c.sameSite;
      // else omit — let Playwright use the browser default
    }
    return n;
  });
  ...
  return {
    ...
    source:     args.cookies_file ? args.cookies_file : "inline",
  };
}

const COLOR_SCHEMES = new Set(["light", "dark", "no-preference", "null"]);

// Thin dedicated wrapper around the viewport-only slice of browser_emulate,
// for callers who don't want emulate's broader (geolocation/color-scheme/
// offline) schema.
async function setViewport(args = {}) {
  requireSessionId(args, "browser_set_viewport");
  const entry = getSession(args.session_id);
  const w = args.width, h = args.height;
  if (!Number.isFinite(w) || !Number.isFinite(h) || w <= 0 || h <= 0)
    throw new ToolError("browser_set_viewport requires positive numeric width/height.", -32602);
  try { await entry.page.setViewportSize({ width: w, height: h }); }
  catch (e) { throw new ToolError(`browser_set_viewport failed: ${e.message}`, -32603); }
  return { session_id: args.session_id, viewport: { width: w, height: h } };
}
 ...
 *
 * Saves the current browser context's Playwright storage state (cookies +
 * per-origin localStorage) to a JSON file on disk.  The saved file can be
 * passed directly to browser_launch's storage_state parameter to resume the
 * same authenticated session in a new browser context — enabling cookie/session
 * persistence across separate automation runs without re-authenticating.
 *
 * Also accepts Playwright storageState files directly back in
 * browser_set_cookies via the cookies_file parameter, since the cookies array
 * is a top-level key inside the same JSON format.
 */
async function saveStorageState(args = {}) {
  requireSessionId(args, "browser_storage_state_save");
  if (!args.path || typeof args.path !== "string" || args.path.trim() === "")
    throw new ToolError("browser_storage_state_save requires a non-empty 'path' string.", -32602);
  ...
  try {
    ...
    throw new ToolError(`browser_storage_state_save: failed to get storage state: ${e.message}`, -32603);
  }

  const pretty = args.pretty !== false; // default true
  ...
  try {
    ...
    throw new ToolError(`browser_storage_state_save: failed to write '${args.path}': ${e.message}`, -32603);
  }

  return {
    session_id:    args.session_id,
    path:          args.path,
    saved_cookies: state.cookies.length,
    saved_origins: state.origins.length,
  };
}

module.exports = {
  getCookies, setCookies, emulate, setViewport, setExtraHeaders,
  getLocalStorage, setLocalStorage, getStorageState, saveStorageState,
};
