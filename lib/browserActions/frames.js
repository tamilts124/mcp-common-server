...
const { ToolError, getSession, requireSessionId, DEFAULT_TIMEOUT, MAX_TIMEOUT, MAX_CONTENT_CHARS } = require("./shared");
...
async function frameClick(args = {}) {
  ...
  const { page } = getSession(args.session_id);
  try {
    await page.frameLocator(args.frame_selector).locator(args.selector).click({ timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) });
  } catch (e) {
...
async function frameType(args = {}) {
  ...
  const { page } = getSession(args.session_id);
  try {
    await page.frameLocator(args.frame_selector).locator(args.selector).fill(String(args.text), { timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) });
  } catch (e) {
...
async function frameGetContent(args = {}) {
  ...
  let content;
  try {
    content = mode === "html" ? await scoped.first().innerHTML({ timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) }) : await scoped.first().innerText({ timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) });
  } catch (e) {
...
async function frameEvaluate(args = {}) {
  ...
  let frame;
  try {
    const handle = await page.locator(args.frame_selector).elementHandle({ timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) });
