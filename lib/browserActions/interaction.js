...
const {
  ...
  DEFAULT_TIMEOUT, MAX_TIMEOUT, requireSessionId,
} = require("./shared");
...
async function click(args = {}) {
  ...
  try {
    await page.click(args.selector, { timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) });
  } catch (e) {
...
async function type(args = {}) {
  ...
  try {
    await page.fill(args.selector, String(args.text), { timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) });
  } catch (e) {
...
async function selectOption(args = {}) {
  ...
  let selected;
  try {
    selected = await page.selectOption(args.selector, opt, { timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) });
  } catch (e) {
...
async function pressKey(args = {}) {
  ...
  try {
    if (args.selector) {
      await page.press(args.selector, args.key, { timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) });
    } else {
...
async function hover(args = {}) {
  ...
  const { page } = getSession(args.session_id);
  try {
    await page.hover(args.selector, { timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) });
  } catch (e) {
...
async function uploadFile(args = {}) {
  ...
  const { page } = getSession(args.session_id);
  try {
    await page.setInputFiles(args.selector, resolvedPaths, { timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) });
  } catch (e) {
...
async function scroll(args = {}) {
  ...
  try {
    if (args.selector) {
      await page.locator(args.selector).scrollIntoViewIfNeeded({ timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) });
    } else {
...
async function doubleClick(args = {}) {
  ...
  const { page } = getSession(args.session_id);
  try {
    await page.dblclick(args.selector, { timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) });
  } catch (e) {
...
async function rightClick(args = {}) {
  ...
  const { page } = getSession(args.session_id);
  try {
    await page.click(args.selector, { button: "right", timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) });
  } catch (e) {
...
async function check(args = {}) {
  ...
  const { page } = getSession(args.session_id);
  try {
    await page.check(args.selector, { timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) });
  } catch (e) {
...
async function uncheck(args = {}) {
  ...
  const { page } = getSession(args.session_id);
  try {
    await page.uncheck(args.selector, { timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) });
  } catch (e) {
