...
async function download(args = {}) {
  ...
  try {
    const [dl] = await Promise.all([
      page.waitForEvent("download", { timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) }),
      page.click(args.selector, { timeout: Math.min(args.timeout || DEFAULT_TIMEOUT, MAX_TIMEOUT) }),
    ]);
    download = dl;
