...
const { findRaceConditionRisk }          = require("./raceConditionOps");
const { findUnvalidatedRedirect }        = require("./unvalidatedRedirectOps");
...
const SCAN_DISPATCH_4 = {
  ...
  find_race_condition_risk(args) {
    const { resolved } = resolveClientPath(args.path || '.');
    const origPath = args.path || '.';
    return findRaceConditionRisk(resolved, origPath, {
      extensions: args.extensions,
      maxResults: args.max_results,
    });
  },

  find_unvalidated_redirect(args) {
    const { resolved } = resolveClientPath(args.path || '.');
    const origPath = args.path || '.';
    return findUnvalidatedRedirect(resolved, origPath, {
      extensions: args.extensions,
      maxResults: args.max_results,
    });
  },
