...
const { gitWorktreeList, gitWorktreePruneCandidates } = require("./gitWorktreeOps");
const { gitTagList } = require("./gitTagOps");
const { suggestNextVersion } = require("./nextVersionOps");
...
const { findStaleTodos } = require("./staleTodosOps");
...
const { checkDotenvFilesNotGitignored } = require("./dotenvExposureOps");
...
const { gitCommitFrequency } = require("./commitFrequencyOps");
const { gitOrphanedBranches } = require("./gitOrphanedBranchesOps");
const { gitSubmoduleStatus } = require("./gitSubmoduleStatusOps");
const { gitWriteOps }        = require("./gitWriteOps");
...
const GIT_DISPATCH = {
  ...
  git_worktree_prune_candidates(args) {
    const repoDir = resolveRepoDir(args.path);
    return gitWorktreePruneCandidates(repoDir);
  },
  ...
  suggest_next_version(args) {
    const repoDir = resolveRepoDir(args.path);
    const origPath = args.path || ".";
    let pkgJsonAbsPath;
    if (args.pkg_path) {
      const { resolved } = resolveClientPath(args.pkg_path);
      pkgJsonAbsPath = resolved;
    } else if (args.path !== undefined) {
      const { resolved } = resolveClientPath(args.path);
      pkgJsonAbsPath = path.join(resolved, "package.json");
    } else {
      pkgJsonAbsPath = path.join(ROOTS[0], "package.json");
    }
    return suggestNextVersion(repoDir, origPath, { pkgJsonAbsPath });
  },
  ...
  check_dotenv_files_not_gitignored(args) {
    const repoDir = resolveRepoDir(args.path);
    return checkDotenvFilesNotGitignored(repoDir, args.path || ".", {
  ...
  find_stale_todos(args) {
    const { resolved, root } = resolveClientPath(args.path);
    const stat = fs.statSync(resolved);
    const isDirectory = stat.isDirectory();
    return findStaleTodos(resolved, args.path, isDirectory, root, {
  ...
  git_commit_frequency(args) {
    const repoDir = resolveRepoDir(args.path);
    return gitCommitFrequency(repoDir, {
      sinceDays:  args.since_days,
      ref:        args.ref || null,
      topAuthors: args.top_authors,
    });
  },

  git_orphaned_branches(args) {
    const repoDir = resolveRepoDir(args.path);
    return gitOrphanedBranches(repoDir, { base: args.base || null });
  },

  git_submodule_status(args) {
    const repoDir = resolveRepoDir(args.path);
    return gitSubmoduleStatus(repoDir);
  },

  git_write_ops(args) {
    const repoDir = resolveRepoDir(args.path);
    return gitWriteOps(args, repoDir, resolveClientPath);
  },
};
