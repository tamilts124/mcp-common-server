...
// git_status  — branch, tracking info, working-tree state
...
// git_diff    — unified diff between two repository states
//
// See also: lib/gitStashOps.js for git_stash_list.
//
// All functions are READ-ONLY and do not require MCP_ALLOW_EXEC=true.
// Security: path traversal is blocked by resolveClientPath in the dispatcher.
const { gitExec, gitExecBuffer, assertSafeArg, q } = require("./gitOpsHelpers");
...
// ── GIT LOG ───────────────────────────────────────────────────────────────────
// gitLog() lives in lib/gitLogOps.js (extracted to stay under this file's
// 500-line threshold) and is re-exported below for backward compatibility
// with callers that `require("./gitOps").gitLog`.
 ...
 * @param {boolean}     statOnly  When true, skip the full unified diff text and
 *                                return only per-file added/deleted line counts
 *                                (delegates to gitDiffStat in ./gitDiffStatOps).
 * @returns {{
 *   fromRef: string,
 *   toRef: string|null,
 *   staged: boolean,
 ...
 * }}
 */
function gitDiff(repoDir, fromRef, toRef, filePath, staged, statOnly) {
  // stat_only mode is handled entirely by the dedicated gitDiffStat module —
  // it does its own ref/file validation and command building, so we delegate
  // before doing any of the full-diff-specific work below.
  if (statOnly) {
    return gitDiffStat(repoDir, fromRef, toRef, filePath, staged);
  }
...
// ── GIT SHOW ──────────────────────────────────────────────────────────────────
/**
 * Return the content of a file as it existed at a specific commit/ref,
 * without checking it out into the working tree.
 *
 * @param {string} repoDir   Absolute path inside (or at) the git working tree.
 * @param {string} ref       Commit/branch/tag ref (e.g. "HEAD", "HEAD~2", "main",
 *                           a hash). Defaults to "HEAD" when omitted/blank.
 * @param {string} filePath  Path to the file, relative to the repo root.
 * @returns {{
 *   ref: string,
 *   resolvedHash: string,
 *   file: string,
 *   size: number,
 *   isBinary: boolean,
 *   content: string|null   // null when isBinary is true
 * }}
 */
function gitShow(repoDir, ref, filePath) {
  const useRef = (ref && ref.trim()) ? ref.trim() : "HEAD";
  assertSafeArg(useRef, "ref");
  assertSafeArg(filePath, "file");

  // Resolve ref to a full commit hash first — validates the ref exists and
  // gives a stable, unambiguous identifier in the result (e.g. "HEAD" or
  // "HEAD~2" resolved to the actual 40-char hash they point at right now).
  let resolvedHash;
  try {
    resolvedHash = gitExec(`rev-parse ${q(`${useRef}^{commit}`)}`, repoDir);
  } catch (e) {
    throw new Error(`git show: unknown ref '${useRef}' — ${e.message.split("\n")[0]}`);
  }

  const treeish = `${resolvedHash}:${filePath}`;

  // Check the object type before trying to read it: distinguishes a missing
  // path from a path that resolves to a directory (tree), both of which
  // would otherwise surface as an opaque `git show` failure.
  let objType;
  try {
    objType = gitExec(`cat-file -t ${q(treeish)}`, repoDir);
  } catch (e) {
    throw new Error(`git show: '${filePath}' does not exist at ref '${useRef}' (${resolvedHash.slice(0, 7)}).`);
  }
  if (objType !== "blob") {
    throw new Error(`git show: '${filePath}' is a ${objType === "tree" ? "directory" : objType}, not a file, at ref '${useRef}' (${resolvedHash.slice(0, 7)}).`);
  }

  const buf = gitExecBuffer(`show ${q(treeish)}`, repoDir);

  // Binary heuristic: a NUL byte anywhere in the first 8000 bytes — the same
  // convention git itself and most diff/grep tools use to decide whether a
  // blob is text or binary.
...
module.exports = { gitStatus, gitLog, gitBlame, gitDiff, gitShow };
