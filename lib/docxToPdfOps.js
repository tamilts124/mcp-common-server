...
//   - Tables (<w:tbl>): equal-width columns, per-cell text (bold/italic/
//     color preserved, one wrapped paragraph per cell), per-cell shading
//     (<w:shd w:fill="RRGGBB"/>), and a stroked border grid around every
//     cell (approximates Word's common "All Borders" table style — exact
//     per-edge border-style/width from <w:tblBorders>/<w:tcBorders> is not
//     modeled, every table renders with a uniform thin black grid).
//
...
function layoutTable(table, cur, y, newPage, usableWidth) {
  ...
  for (const row of rows) {
    const cellLines = row.map(cell => {
      ...
      return wrapRuns(runs, BODY_SIZE, colWidth - 2 * CELL_PAD);
    });
    ...
    for (let c = 0; c < row.length; c++) {
      const cx = MARGIN + c * colWidth;
      const fill = row[c].fill;
      if (fill) cur.push({ type: "rect", x: cx, y: rowTop - rowHeight, w: colWidth, h: rowHeight, fill });
      cur.push({ type: "strokeRect", x: cx, y: rowTop - rowHeight, w: colWidth, h: rowHeight });
