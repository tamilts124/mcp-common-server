...
- [x] Add markdown_to_html + xml_parse tools — status: tested (75/75, v4.144.0)
  - notes: markdownHtmlOps.js (zero-dep MD→HTML: ATX/setext headings, fenced/indented code, blockquotes, ordered/unordered lists, GFM tables, thematic breaks, hard line breaks, bold/italic/strike/code/links/images/autolinks). xmlParseOps.js (zero-dep recursive-descent XML→JSON AST: declarations, elements, attrs, text, CDATA, comments, processing instructions, self-closing, namespaces, entity decoding, dot-notation path query with attrs.KEY support). Both wired in dispatchRead.js + utilSchemas8.js chained into utilSchemas.js. 75/75 tests pass across all 5 rigor levels (A-J).